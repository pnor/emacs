...
static NSColor *transparent_foreground_ns_lookup_indexed_color(struct face *face, struct frame *f) {
  NSColor *base_color = ns_lookup_indexed_color(NS_FACE_FOREGROUND(face), f);
  return [base_color colorWithAlphaComponent:f->alpha_background];
}
...
+ (NSColor *)colorForEmacsRed:(CGFloat)red
                        green:(CGFloat)green
                         blue:(CGFloat)blue
                        alpha:(CGFloat)alpha {
#if defined(NS_IMPL_COCOA) && MAC_OS_X_VERSION_MAX_ALLOWED >= 1070
  if (ns_use_srgb_colorspace && NSAppKitVersionNumber >= NSAppKitVersionNumber10_7)
...
static unsigned convert_ns_to_X_keysym[] = {
    ...
    0x09,
    0x19,
    0x09, /* left tab->regular since pass shift */
    ...
    0x41 | NSEventModifierFlagNumericPad,
    0xAE, /* KP_Decimal */
    0x43 | NSEventModifierFlagNumericPad,
    0xAA, /* KP_Multiply */
    0x45 | NSEventModifierFlagNumericPad,
    0xAB, /* KP_Add */
    0x4B | NSEventModifierFlagNumericPad,
    0xAF, /* KP_Divide */
    0x4E | NSEventModifierFlagNumericPad,
    0xAD, /* KP_Subtract */
    0x51 | NSEventModifierFlagNumericPad,
    0xBD, /* KP_Equal */
    0x52 | NSEventModifierFlagNumericPad,
    0xB0, /* KP_0 */
    0x53 | NSEventModifierFlagNumericPad,
    0xB1, /* KP_1 */
    0x54 | NSEventModifierFlagNumericPad,
    0xB2, /* KP_2 */
    0x55 | NSEventModifierFlagNumericPad,
    0xB3, /* KP_3 */
    0x56 | NSEventModifierFlagNumericPad,
    0xB4, /* KP_4 */
    0x57 | NSEventModifierFlagNumericPad,
    0xB5, /* KP_5 */
    0x58 | NSEventModifierFlagNumericPad,
    0xB6, /* KP_6 */
    0x59 | NSEventModifierFlagNumericPad,
    0xB7, /* KP_7 */
    0x5B | NSEventModifierFlagNumericPad,
    0xB8, /* KP_8 */
    0x5C | NSEventModifierFlagNumericPad,
    0xB9, /* KP_9 */
...
static NSMutableArray *ns_pending_files, *ns_pending_service_names, *ns_pending_service_args;
...
static Lisp_Object mod_of_kind(Lisp_Object modifier, Lisp_Object kind) {
  if (SYMBOLP(modifier))
    return modifier;
  else {
...
static unsigned int ev_modifiers_helper(unsigned int flags, unsigned int left_mask,
  ...
  if (flags & either_mask) {
    ...
    BOOL right_key = (flags & right_mask) == right_mask && !EQ(right_modifier, Qleft);
...
#define EV_MODIFIERS2(flags, kind)                                                                 \
  (((flags & NSEventModifierFlagHelp) ? hyper_modifier : 0) |                                      \
   ((flags & NSEventModifierFlagShift) ? shift_modifier : 0) |                                     \
   ((flags & NS_FUNCTION_KEY_MASK)                                                                 \
        ? parse_solitary_modifier(mod_of_kind(ns_function_modifier, kind))                         \
        : 0) |                                                                                     \
   ev_modifiers_helper(flags, NSLeftControlKeyMask, NSRightControlKeyMask,                         \
                       NSEventModifierFlagControl, mod_of_kind(ns_control_modifier, kind),         \
                       mod_of_kind(ns_right_control_modifier, kind)) |                             \
   ev_modifiers_helper(flags, NSLeftCommandKeyMask, NSRightCommandKeyMask,                         \
                       NSEventModifierFlagCommand, mod_of_kind(ns_command_modifier, kind),         \
                       mod_of_kind(ns_right_command_modifier, kind)) |                             \
   ev_modifiers_helper(flags, NSLeftAlternateKeyMask, NSRightAlternateKeyMask,                     \
                       NSEventModifierFlagOption, mod_of_kind(ns_alternate_modifier, kind),        \
                       mod_of_kind(ns_right_alternate_modifier, kind)))
...
#define EV_BUTTON(e)                                                                               \
  ((([e type] == NSEventTypeLeftMouseDown) || ([e type] == NSEventTypeLeftMouseUp))                \
       ? 0                                                                                         \
...
#define FRAME_DECORATED_FLAGS                                                                      \
  (NSWindowStyleMaskTitled | NSWindowStyleMaskResizable | NSWindowStyleMaskMiniaturizable |        \
...
void ns_init_events(struct input_event *ev) {
  EVENT_INIT(*ev);
  emacs_event = ev;
}

void ns_finish_events(void) { emacs_event = NULL; }

static void hold_event(struct input_event *event) {
  if (hold_event_q.nr == hold_event_q.cap) {
    if (hold_event_q.cap == 0)
      hold_event_q.cap = 10;
    else
      hold_event_q.cap *= 2;
    hold_event_q.q = xrealloc(hold_event_q.q, hold_event_q.cap * sizeof *hold_event_q.q);
  }
...
const char *ns_relocate(const char *epath)
   ...
   directories are separated by colons.  */
{
  ...
  NSString *fixedPath =
      [NSString pathWithComponents:[NSArray arrayWithObjects:root, original, nil]];
  NSFileManager *fileManager = [NSFileManager defaultManager];

  if (![original isAbsolutePath] && [fileManager fileExistsAtPath:fixedPath isDirectory:NULL])
   ...
   from the GUI. Get the locale from the OS and set LANG.  */
{
  ...
  @try {
    /* It seems macOS should probably use UTF-8 everywhere.
       ...
       so hard-code '.UTF-8'.  */
    NSString *localeID = [NSString stringWithFormat:@"%@.UTF-8", [locale localeIdentifier]];
...
void ns_release_object(void *obj)
/* --------------------------------------------------------------------------
...
void ns_retain_object(void *obj)
/* --------------------------------------------------------------------------
...
void *ns_alloc_autorelease_pool(void)
/* --------------------------------------------------------------------------
...
static BOOL ns_menu_bar_should_be_hidden(void)
...
static struct EmacsMargins ns_screen_margins(NSScreen *screen)
/* The parts of SCREEN used by the operating system.  */
{
  ...
  if (ns_menu_bar_should_be_hidden()) {
    ...
    CGFloat visibleFrameTop = (screenVisibleFrame.origin.y + screenVisibleFrame.size.height);
...

  {
    ...
    CGFloat visibleFrameRight = (screenVisibleFrame.origin.x + screenVisibleFrame.size.width);
  ...
  NSTRACE_MSG("left:%g right:%g top:%g bottom:%g", margins.left, margins.right, margins.top,
              margins.bottom);
...
static struct EmacsMargins ns_screen_margins_ignoring_hidden_dock(NSScreen *screen)
...
static CGFloat ns_menu_bar_height(NSScreen *screen)
...
static inline NSRect ns_parent_window_rect(struct frame *f) {
  NSRect parentRect;

  if (FRAME_PARENT_FRAME(f) != NULL) {
    EmacsView *parentView = FRAME_NS_VIEW(FRAME_PARENT_FRAME(f));
    parentRect = [parentView convertRect:[parentView frame] toView:nil];
...
static NSRect ns_row_rect(struct window *w, struct glyph_row *row, enum glyph_row_area area)
...
double ns_frame_scale_factor(struct frame *f) {
...
static NSRect constrain_frame_rect(NSRect frameRect, bool isFullscreen) {
  NSTRACE("constrain_frame_rect(" NSTRACE_FMT_RECT ")", NSTRACE_ARG_RECT(frameRect));
  ...
  for (i = 0; i < nr_screens; ++i) {
    ...
    NSTRACE_MSG("Screen %d: " NSTRACE_FMT_RECT, i, NSTRACE_ARG_RECT(scrRect));
  ...
  if (multiscreenRect.size.width == 0 || multiscreenRect.size.height == 0) {
  ...
  if (ns_menu_bar_should_be_hidden()) {
    ...
    CGFloat topOfWorkArea =
        (multiscreenRect.origin.y + multiscreenRect.size.height - menu_bar_height);
...
static void ns_constrain_all_frames(void)
/* --------------------------------------------------------------------------
  ...
  FOR_EACH_FRAME(tail, frame) {
    ...
    if (FRAME_NS_P(f)) {
      ...
      if (![view isFullscreen]) {
        [[view window] setFrame:constrain_frame_rect([[view window] frame], false) display:NO];
      }
    }
  }
...
static void ns_update_auto_hide_menu_bar(void)
/* --------------------------------------------------------------------------
...
static void ns_update_begin(struct frame *f)
/* --------------------------------------------------------------------------
...
static void ns_update_end(struct frame *f)
/* --------------------------------------------------------------------------
...
static void ns_focus(struct frame *f, NSRect *r, int n)
/* --------------------------------------------------------------------------
...
static void ns_unfocus(struct frame *f)
/* --------------------------------------------------------------------------
...
- (id)init {
  ...
  if ((self = [super init])) {
    ...
    [self.image setSize:NSMakeSize(self.image.size.width * 5, self.image.size.height * 5)];
#endif
  }
  return self;
}
...
- (void)show:(NSView *)view {
  ...
  if (nestCount == 0) {
    ...
    isAttached = true;
    mView = view;
    [[[view window] contentView] addSubview:self positioned:NSWindowAbove relativeTo:nil];
  }
...
static void ns_ring_bell(struct frame *f)
/* --------------------------------------------------------------------------
     "Beep" routine
   -------------------------------------------------------------------------- */
{
...
static Lisp_Object ns_get_focus_frame(struct frame *f)
/* --------------------------------------------------------------------------
     External (hook)
   -------------------------------------------------------------------------- */
{
...
static void ns_focus_frame(struct frame *f, bool noactivate)
/* --------------------------------------------------------------------------
     External (hook)
   -------------------------------------------------------------------------- */
{
...
static void ns_raise_frame(struct frame *f, BOOL make_key)
/* --------------------------------------------------------------------------
...
static void ns_lower_frame(struct frame *f)
/* --------------------------------------------------------------------------
     Send window to back
   -------------------------------------------------------------------------- */
{
...
static void ns_frame_raise_lower(struct frame *f, bool raise)
/* --------------------------------------------------------------------------
     External (hook)
   -------------------------------------------------------------------------- */
{
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (dpyinfo->ns_focus_frame) {
    dpyinfo->highlight_frame = (FRAMEP(FRAME_FOCUS_FRAME(dpyinfo->ns_focus_frame))
  ...
  if (dpyinfo->highlight_frame && dpyinfo->highlight_frame != old_highlight) {
    if (old_highlight) {
...
static void ns_make_frame_invisible(struct frame *f)
/* --------------------------------------------------------------------------
...
static void ns_make_frame_visible_invisible(struct frame *f, bool visible)
/* --------------------------------------------------------------------------
     External (hook)
   -------------------------------------------------------------------------- */
{
...
void ns_free_frame_resources(struct frame *f) {
  NSView *view;
  struct ns_display_info *dpyinfo;
  Mouse_HLInfo *hlinfo;
...
static void ns_destroy_window(struct frame *f)
/* --------------------------------------------------------------------------
...
static void ns_set_window_size(struct frame *f, bool change_gravity, int width, int height)
/* --------------------------------------------------------------------------
...
void ns_set_undecorated(struct frame *f, Lisp_Object new_value, Lisp_Object old_value)
/* --------------------------------------------------------------------------
...
void ns_set_parent_frame(struct frame *f, Lisp_Object new_value, Lisp_Object old_value)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (!NILP(new_value) &&
      (!FRAMEP(new_value) || !FRAME_LIVE_P(p = XFRAME(new_value)) || !FRAME_NS_P(p))) {
...
void ns_set_no_focus_on_map(struct frame *f, Lisp_Object new_value, Lisp_Object old_value)
...
void ns_set_no_accept_focus(struct frame *f, Lisp_Object new_value, Lisp_Object old_value)
...
void ns_set_z_group(struct frame *f, Lisp_Object new_value, Lisp_Object old_value)
...
#ifdef NS_IMPL_COCOA
void ns_set_appearance(struct frame *f, Lisp_Object new_value, Lisp_Object old_value) {
...
void ns_set_transparent_titlebar(struct frame *f, Lisp_Object new_value, Lisp_Object old_value) {
#if MAC_OS_X_VERSION_MAX_ALLOWED >= 101000
  EmacsView *view = (EmacsView *)FRAME_NS_VIEW(f);
  NSWindow *window = [view window];
...
static void ns_fullscreen_hook(struct frame *f) {
  EmacsView *view = (EmacsView *)FRAME_NS_VIEW(f);
...
NSColor *ns_lookup_indexed_color(unsigned long idx, struct frame *f) {
...
unsigned long ns_index_color(NSColor *color, struct frame *f) {
  struct ns_color_table *color_table = FRAME_DISPLAY_INFO(f)->color_table;
  ptrdiff_t idx;
  ptrdiff_t i;
  ...
  if ([color_table->empty_indices count] > 0) {
    ...
    if (color_table->avail == color_table->size)
      color_table->colors = xpalloc(color_table->colors, &color_table->size, 1,
...
static int ns_get_color(const char *name, NSColor **col)
/* --------------------------------------------------------------------------
     Parse a color name
   -------------------------------------------------------------------------- */
   ...
   See https://lists.gnu.org/r/emacs-devel/2009-07/msg01203.html.  */
{
...
#ifdef NS_IMPL_COCOA
    NSString *defname = [[NSUserDefaults standardUserDefaults] stringForKey:@"AppleHighlightColor"];
    if (defname != nil)
      nsname = defname;
    else
#endif
  ...
  if (parse_color_spec(name, &r16, &g16, &b16)) {
    *col =
        [NSColor colorForEmacsRed:r16 / 65535.0 green:g16 / 65535.0 blue:b16 / 65535.0 alpha:1.0];
    unblock_input();
    return 0;
    ...
    float r, g, b;
    if ([scanner scanFloat:&r] && r >= 0 && r <= 1 && [scanner scanFloat:&g] && g >= 0 && g <= 1 &&
  ...
  /* Otherwise, color is expected to be from a list */
  {
    ...
    if ([nsname compare:@"Highlight" options:NSCaseInsensitiveSearch] == NSOrderedSame)
    ...
    while ((clist = [lenum nextObject]) && new == nil) {
      ...
      while ((name = [cenum nextObject]) && new == nil) {
        if ([name compare:nsname options:NSCaseInsensitiveSearch] == NSOrderedSame)
...
unsigned long ns_color_index_to_rgba(int idx, struct frame *f) {
  NSColor *col;
  ...
  return ARGB_TO_ULONG((unsigned long)(a * 255), (unsigned long)(r * 255), (unsigned long)(g * 255),
                       (unsigned long)(b * 255));
}

void ns_query_color(void *col, Emacs_Color *color_def, bool setPixel)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (setPixel == YES)
    color_def->pixel = ARGB_TO_ULONG((unsigned long)(a * 255), (unsigned long)(r * 255),
                                     (unsigned long)(g * 255), (unsigned long)(b * 255));
}

bool ns_defined_color(struct frame *f, const char *name, Emacs_Color *color_def, bool alloc,
                      bool makeIndex)
/* --------------------------------------------------------------------------
...
static void ns_query_frame_background_color(struct frame *f, Emacs_Color *bgcolor)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  CGPoint mouse_pos =
      CGPointMake(f->left_pos + pix_x,
                  f->top_pos + pix_y + FRAME_NS_TITLEBAR_HEIGHT(f) + FRAME_TOOLBAR_HEIGHT(f));
  CGWarpMouseCursorPosition(mouse_pos);
#endif
}

static int ns_note_mouse_movement(struct frame *frame, CGFloat x, CGFloat y, BOOL dragging)
/*   ------------------------------------------------------------------------
     ...
     ------------------------------------------------------------------------ */
{
  ...
  if (force_update || x < r->origin.x || x >= r->origin.x + r->size.width || y < r->origin.y ||
...
static void ns_mouse_position(struct frame **fp, int insist, Lisp_Object *bar_window,
   ...
   -------------------------------------------------------------------------- */
{
  ...
  do {
    ...
    window_number =
        [NSWindow windowNumberAtPoint:screen_position belowWindowWithWindowNumber:window_number];
  ...
  if (!f && EQ(track_mouse, Qdropping) && dpyinfo->last_mouse_frame &&
  ...
  if (f && FRAME_NS_P(f)) {
    ...
    remember_mouse_glyph(f, view_position.x, view_position.y, &dpyinfo->last_mouse_glyph);
    ...
    if (x)
      XSETINT(*x, lrint(view_position.x));
    if (y)
      XSETINT(*y, lrint(view_position.y));
    if (time)
...
static void ns_frame_up_to_date(struct frame *f)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (FRAME_NS_P(f)) {
    ...
    if (f == hlinfo->mouse_face_mouse_frame) {
      block_input();
      ns_update_begin(f);
      note_mouse_highlight(hlinfo->mouse_face_mouse_frame, hlinfo->mouse_face_mouse_x,
...
static void ns_define_frame_cursor(struct frame *f, Emacs_Cursor cursor)
/* --------------------------------------------------------------------------
...
static unsigned ns_convert_key(unsigned code)
/* --------------------------------------------------------------------------
...
char *get_keysym_name(int keysym)
/* --------------------------------------------------------------------------
...
#ifdef NS_IMPL_COCOA
static Lisp_Object right_mod(Lisp_Object left, Lisp_Object right) {
  return EQ(right, Qleft) ? left : right;
}

static bool nil_or_none(Lisp_Object val) { return NILP(val) || EQ(val, Qnone); }

static UniChar ns_get_shifted_character(NSEvent *event)
   ...
   Carbon a requirement.  */
{
  ...
  if ((flags & NSRightAlternateKeyMask) == NSRightAlternateKeyMask &&
      nil_or_none(
          mod_of_kind(right_mod(ns_alternate_modifier, ns_right_alternate_modifier), QCordinary)))
  ...
  if ((flags & NSRightCommandKeyMask) == NSRightCommandKeyMask &&
      nil_or_none(
          mod_of_kind(right_mod(ns_command_modifier, ns_right_command_modifier), QCordinary)))
  ...
  result = UCKeyTranslate(layout, [event keyCode], kUCKeyActionDown, (modifiers >> 8) & 0xFF,
                          LMGetKbdType(), kUCKeyTranslateNoDeadKeysBit, &dead_key_state,
                          max_string_length, &actual_string_length, buf);

  if (result != 0) {
    NSLog(@"Failed to translate character '%@' with modifiers %x", [event characters], modifiers);
    return 0;
  }
...
void ns_clear_frame(struct frame *f)
/* --------------------------------------------------------------------------
...
static void ns_clear_frame_area(struct frame *f, int x, int y, int width, int height)
/* --------------------------------------------------------------------------
...
static void ns_scroll_run(struct window *w, struct run *run)
/* --------------------------------------------------------------------------
...
static void ns_clear_under_internal_border(struct frame *f) {
  ...
  if (FRAME_LIVE_P(f) && FRAME_INTERNAL_BORDER_WIDTH(f) > 0) {
    ...
    int face_id =
        (FRAME_PARENT_FRAME(f)
             ...
             : (!NILP(Vface_remapping_alist) ? lookup_basic_face(NULL, f, INTERNAL_BORDER_FACE_ID)
...
static void ns_after_update_window_line(struct window *w, struct glyph_row *desired_row)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (windows_or_buffers_changed && desired_row->full_width_p &&
      (f = XFRAME(w->frame), width = FRAME_INTERNAL_BORDER_WIDTH(f), width != 0) &&
      (height = desired_row->visible_height, height > 0)) {
    int y = WINDOW_TO_FRAME_PIXEL_Y(w, max(0, desired_row->y));
    int face_id = !NILP(Vface_remapping_alist) ? lookup_basic_face(NULL, f, INTERNAL_BORDER_FACE_ID)
                                               : INTERNAL_BORDER_FACE_ID;
    struct face *face = FACE_FROM_ID_OR_NULL(f, face_id);
    ...
    if (face) {
      ...
      NSRectFill(NSMakeRect(FRAME_PIXEL_WIDTH(f) - width, y, width, height));

      ns_unfocus(f);
    } else {
      ns_clear_frame_area(f, 0, y, width, height);
      ns_clear_frame_area(f, FRAME_PIXEL_WIDTH(f) - width, y, width, height);
    }
    unblock_input();
  }
}

static void ns_shift_glyphs_for_insert(struct frame *f, int x, int y, int width, int height,
                                       int shift_by)
/* --------------------------------------------------------------------------
...
static void ns_compute_glyph_string_overhangs(struct glyph_string *s)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (s->char2b) {
    ...
    if (s->first_glyph->type == CHAR_GLYPH && !s->font_not_found_p) {
      ...
      s->right_overhang = metrics.rbearing > metrics.width ? metrics.rbearing - metrics.width : 0;
    ...
    if (EQ(font->driver->type, Qns))
      s->right_overhang = ((struct nsfont_info *)font)->ital ? FONT_HEIGHT(font) * 0.2 : 0;
    else
#endif
      s->right_overhang = 0;
  }
...
static void ns_define_fringe_bitmap(int which, unsigned short *bits, int h, int w) {
...
static void ns_destroy_fringe_bitmap(int which) {
  [fringe_bmp removeObjectForKey:[NSNumber numberWithInt:which]];
}

static void ns_draw_fringe_bitmap(struct window *w, struct glyph_row *row,
   ...
   -------------------------------------------------------------------------- */
{
  ...
  NSTRACE_MSG("which:%d cursor:%d overlay:%d width:%d height:%d period:%d", p->which, p->cursor_p,
              p->overlay_p, p->wd, p->h, p->dh);
...
static void ns_draw_window_cursor(struct window *w, struct glyph_row *glyph_row, int x, int y,
                                  enum text_cursor_kinds cursor_type, int cursor_width, bool on_p,
                                  bool active_p)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if ((phys_cursor_glyph = get_phys_cursor_glyph(w)) == NULL) {
    if (glyph_row->exact_window_width_line_p && w->phys_cursor.hpos >= glyph_row->used[TEXT_AREA]) {
  ...
  if (face && NS_FACE_BACKGROUND(face) == ns_index_color(FRAME_CURSOR_COLOR(f), f)) {
...
static void ns_draw_vertical_window_border(struct window *w, int x, int y0, int y1)
/* --------------------------------------------------------------------------
...
static void ns_draw_window_divider(struct window *w, int x0, int x1, int y0, int y1)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  unsigned long color_first = (face_first ? face_first->foreground : FRAME_FOREGROUND_PIXEL(f));
  unsigned long color_last = (face_last ? face_last->foreground : FRAME_FOREGROUND_PIXEL(f));
...
static void ns_show_hourglass(struct frame *f) {
  /* TODO: add NSProgressIndicator to all frames.  */
}

static void ns_hide_hourglass(struct frame *f) {
...
static int ns_get_glyph_string_clip_rect(struct glyph_string *s, NativeRectangle *nr)
/* --------------------------------------------------------------------------
...
static void ns_draw_underwave(struct glyph_string *s, EmacsCGFloat width, EmacsCGFloat x) {
...
static void ns_draw_text_decoration(struct glyph_string *s, struct face *face, NSColor *defaultCol,
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (face->underline) {
    if (s->face->underline == FACE_UNDER_WAVE) {
      ...
      if (s->prev && s->prev->face->underline == FACE_UNDER_LINE &&
        ...
        Lisp_Object val = (WINDOW_BUFFER_LOCAL_VALUE(Qunderline_minimum_offset, s->w));
        ...
        val = (WINDOW_BUFFER_LOCAL_VALUE(Qx_underline_at_descent_line, s->w));
        underline_at_descent_line = !(NILP(val) || EQ(val, Qunbound));

        val = (WINDOW_BUFFER_LOCAL_VALUE(Qx_use_underline_position_properties, s->w));
        use_underline_position_properties = !(NILP(val) || EQ(val, Qunbound));

        /* Use underline thickness of font, defaulting to 1.  */
        thickness = (font && font->underline_thickness > 0) ? font->underline_thickness : 1;

        /* Determine the offset of underlining from the baseline.  */
        if (underline_at_descent_line)
          position = descent - thickness;
        else if (use_underline_position_properties && font && font->underline_position >= 0)
...
static void ns_draw_box(NSRect r, CGFloat hthickness, CGFloat vthickness, NSColor *col, char left_p,
                        char right_p)
/* --------------------------------------------------------------------------
...
static void ns_draw_relief(NSRect outer, int hthickness, int vthickness, char raised_p, char top_p,
                           char bottom_p, char left_p, char right_p, struct glyph_string *s)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  inner = NSMakeRect(NSMinX(outer) + (left_p ? hthickness : 0),
                     ...
                     NSWidth(outer) - (left_p ? hthickness : 0) - (right_p ? hthickness : 0),
                     NSHeight(outer) - (top_p ? vthickness : 0) - (bottom_p ? vthickness : 0));
...
static void ns_dumpglyphs_box_or_relief(struct glyph_string *s)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (s->hl == DRAW_MOUSE_FACE) {
    face = FACE_FROM_ID_OR_NULL(s->f, MOUSE_HL_INFO(s->f)->mouse_face_face_id);
    if (!face)
  ...
  last_x = ((s->row->full_width_p && !s->w->pseudo_window_p) ? WINDOW_RIGHT_EDGE_X(s->w)
                                                             : window_box_right(s->w, s->area));
  if (s->cmp || s->img)
    last_glyph = s->first_glyph;
  else if (s->first_glyph->type == COMPOSITE_GLYPH && s->first_glyph->u.cmp.automatic) {
    struct glyph *end = s->row->glyphs[s->area] + s->row->used[s->area];
    struct glyph *g = s->first_glyph;
    for (last_glyph = g++;
         g < end && g->u.cmp.automatic && g->u.cmp.id == s->cmp_id && g->slice.cmp.to < s->cmp_to;
         last_glyph = g++)
      ;
  } else
  ...
  left_p = (s->first_glyph->left_box_line_p ||
            (s->hl == DRAW_MOUSE_FACE && (s->prev == NULL || s->prev->hl != s->hl)));
  right_p = (last_glyph->right_box_line_p ||
             (s->hl == DRAW_MOUSE_FACE && (s->next == NULL || s->next->hl != s->hl)));
  ...
  if (s->face->box == FACE_SIMPLE_BOX && s->face->box_color) {
    ...
    ns_draw_relief(r, abs(hthickness), abs(vthickness), s->face->box == FACE_RAISED_BOX, 1, 1,
                   left_p, right_p, s);
  }
}

static void ns_maybe_dumpglyphs_background(struct glyph_string *s, char force_p)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (!s->background_filled_p /* || s->hl == DRAW_MOUSE_FACE*/) {
    ...
    if (FONT_HEIGHT(s->font) < s->height - 2 * box_line_width
        ...
        || FONT_TOO_HIGH(s->font) || s->font_not_found_p || s->extends_to_end_of_line_p ||
        force_p) {
      struct face *face;
      if (s->hl == DRAW_MOUSE_FACE) {
      ...
      if (s->hl != DRAW_CURSOR) {
        NSRect r = NSMakeRect(s->x, s->y + box_line_width, s->background_width,
...
static void ns_dumpglyphs_image(struct glyph_string *s, NSRect r)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (s->face->box != FACE_NO_BOX && s->first_glyph->left_box_line_p && s->slice.x == 0)
  ...
  if (bg_height > s->slice.height || s->img->hmargin || s->img->vmargin || s->img->mask ||
  ...
  if (s->img->relief || s->hl == DRAW_IMAGE_RAISED || s->hl == DRAW_IMAGE_SUNKEN) {
    if (s->hl == DRAW_IMAGE_SUNKEN || s->hl == DRAW_IMAGE_RAISED) {
      th = (tool_bar_button_relief < 0 ? DEFAULT_TOOL_BAR_BUTTON_RELIEF
    ...
    ns_draw_relief(r, th, th, raised_p, s->slice.y == 0,
                   s->slice.y + s->slice.height == s->img->height, s->slice.x == 0,
...
static void ns_dumpglyphs_stretch(struct glyph_string *s) {
  NSRect r[2];
  NSRect glyphRect;
  int n;
  struct face *face;
  ...
  if (!s->background_filled_p) {
    ...
    ns_draw_text_decoration(s, face, fgCol, NSWidth(glyphRect), NSMinX(glyphRect));
...
static void ns_draw_glyph_string_foreground(struct glyph_string *s) {
  int x, flags;
  struct font *font = s->font;
  ...
  flags = s->hl == DRAW_CURSOR
              ? NS_DUMPGLYPH_CURSOR
              : (s->hl == DRAW_MOUSE_FACE
                     ? NS_DUMPGLYPH_MOUSEFACE
                     : (s->for_overlaps ? NS_DUMPGLYPH_FOREGROUND : NS_DUMPGLYPH_NORMAL));
...
static void ns_draw_composite_glyph_string_foreground(struct glyph_string *s) {
  int i, j, x;
  struct font *font = s->font;
...
static void ns_draw_glyph_string(struct glyph_string *s)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (!s->for_overlaps && s->face->box != FACE_NO_BOX &&
      (s->first_glyph->type == CHAR_GLYPH || s->first_glyph->type == COMPOSITE_GLYPH)) {
    n = ns_get_glyph_string_clip_rect(s, r);
    ns_focus(s->f, r, n);
  ...
  case COMPOSITE_GLYPH:
    ...
    if (s->for_overlaps || (s->cmp_from > 0 && !s->first_glyph->u.cmp.automatic)) {
      s->background_filled_p = 1;
    } else {
      ns_maybe_dumpglyphs_background(s, s->first_glyph->type == COMPOSITE_GLYPH);
    }
...
static void ns_send_appdefined(int value)
/* --------------------------------------------------------------------------
...
static int ns_read_socket(struct terminal *terminal, struct input_event *hold_quit)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if ([NSThread isMainThread]) {
    ...
    /* Deal with pending service requests.  */
             ...
             [(EmacsApp *)NSApp fulfillService:[ns_pending_service_names objectAtIndex:0]
...
int ns_select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
   ...
   -------------------------------------------------------------------------- */
{
  ...
  for (k = 0; k < nfds; k++) {
    if (readfds && FD_ISSET(k, readfds))
      ++nr;
    if (writefds && FD_ISSET(k, writefds))
      ++nr;
  }

  if (NSApp == nil || ![NSThread isMainThread] ||
      (timeout && timeout->tv_sec == 0 && timeout->tv_nsec == 0))
    return thread_select(pselect, nfds, readfds, writefds, exceptfds, timeout, sigmask);
  else {
...
static void ns_set_vertical_scroll_bar(struct window *window, int portion, int whole, int position)
/* --------------------------------------------------------------------------
...
static void ns_set_horizontal_scroll_bar(struct window *window, int portion, int whole,
                                         int position)
/* --------------------------------------------------------------------------
      External (hook): Update or add scrollbar.
   -------------------------------------------------------------------------- */
{
  ...
  /* If there are both horizontal and vertical scroll-bars they leave
     ...
     it fills with junk.  */
  if (!NILP(window->vertical_scroll_bar))
    ns_clear_frame_area(f, WINDOW_SCROLL_BAR_AREA_X(window), top, NS_SCROLL_BAR_HEIGHT(f), height);
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if (!NILP(window->vertical_scroll_bar) && WINDOW_HAS_VERTICAL_SCROLL_BAR(window)) {
    bar = XNS_SCROLL_BAR(window->vertical_scroll_bar);
    [bar reprieve];
  }

  if (!NILP(window->horizontal_scroll_bar) && WINDOW_HAS_HORIZONTAL_SCROLL_BAR(window)) {
    bar = XNS_SCROLL_BAR(window->horizontal_scroll_bar);
    [bar reprieve];
  }
}
...
static void ns_free_pixmap(struct frame *_f, Emacs_Pixmap pixmap) { ns_release_object(pixmap); }
...
int ns_display_pixel_height(struct ns_display_info *dpyinfo) {
  NSArray *screens = [NSScreen screens];
...
int ns_display_pixel_width(struct ns_display_info *dpyinfo) {
  NSArray *screens = [NSScreen screens];
  NSEnumerator *enumerator = [screens objectEnumerator];
  NSScreen *screen;
  NSRect frame;
...
static void ns_default(const char *parameter, Lisp_Object *result, Lisp_Object yesval,
                       Lisp_Object noval, BOOL is_float, BOOL is_modstring)
/* --------------------------------------------------------------------------
  ...
  if (value) {
    ...
    if (c_strcasecmp(value, "YES") == 0)
      ...
      *result = ns_string_to_lispmod(value);
      ...
      fprintf(stderr, "Bad value for default \"%s\": \"%s\"\n", parameter, value);
  }
}

static void ns_initialize_display_info(struct ns_display_info *dpyinfo)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  dpyinfo->resy = 72.27;
  dpyinfo->color_p = ![NSDeviceWhiteColorSpace isEqualToString:NSColorSpaceFromDepth(depth)] &&
                     ![NSCalibratedWhiteColorSpace isEqualToString:NSColorSpaceFromDepth(depth)];
...
static void ns_default_font_parameter(struct frame *f, Lisp_Object parms) {}
...
static Lisp_Object ns_new_font(struct frame *f, Lisp_Object font_object, int fontset);

static struct terminal *ns_create_terminal(struct ns_display_info *dpyinfo)
/* --------------------------------------------------------------------------
...
struct ns_display_info *ns_term_init(Lisp_Object display_name)
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
{
  ...
  [NSThread detachNewThreadSelector:@selector(fd_handler:) toTarget:NSApp withObject:nil];
  ...
  if (!inhibit_x_resources) {
    ns_default("GSFontAntiAlias", &ns_antialias_text, Qt, Qnil, NO, NO);
    tmp = Qnil;
    /* this is a standard variable */
    ns_default("AppleAntiAliasingThreshold", &tmp, make_float(10.0), make_float(6.0), YES, NO);
...

  {
    ...
    if (cl == nil || [[cl allKeys] count] < 752) {
      ...
      color_file =
          Fexpand_file_name(build_string("rgb.txt"), Fsymbol_value(intern("data-directory")));
      ...
      for (; CONSP(color_map); color_map = XCDR(color_map)) {
        ...
        [cl setColor:[NSColor colorForEmacsRed:RED_FROM_ULONG(c) / 255.0
  ...
  ns_return_types = [[NSArray arrayWithObjects:NSPasteboardTypeString, nil] retain];
  ns_drag_types = [[NSArray arrayWithObjects:NSPasteboardTypeString, NSPasteboardTypeTabularText,
...
- (void)logNotification:(NSNotification *)notification {
  ...
  if (!strstr(name, "Update") && !strstr(name, "NSMenu") && !strstr(name, "WindowNumber"))
   ...
   -------------------------------------------------------------------------- */
{
  ...
  if ([NSApp activationPolicy] == NSApplicationActivationPolicyProhibited) {
    ...
    [NSApp
        setApplicationIconImage:[EmacsImage
                                    allocInitFromFile:build_string(
...
static bool runAlertPanel(NSString *title, NSString *msgFormat, NSString *defaultButton,
...
static int not_in_argv(NSString *arg) {
  int k;
  const char *a = [arg UTF8String];
  for (k = 1; k < initial_argc; ++k)
    if (strcmp(a, initial_argv[k]) == 0)
      return 0;
  return 1;
}
...
- (void)requestService:(NSPasteboard *)pboard
  ...
  [ns_pending_service_args
      addObject:[NSString stringWithLispString:ns_string_from_pasteboard(pboard)]];
}
...
- (void)setMarkedText:(id)aString selectedRange:(NSRange)selRange {
  NSString *str = [aString respondsToSelector:@selector(string)] ? [aString string] : aString;

  NSTRACE("[EmacsView setMarkedText:selectedRange:]");

  if (NS_KEYLOG)
    NSLog(@"setMarkedText '%@' len =%lu range %lu from %lu", str, (unsigned long)[str length],
          (unsigned long)selRange.length, (unsigned long)selRange.location);
...
- (NSRange)markedRange {
  ...
  NSRange rng =
      workingText != nil ? NSMakeRange(0, [workingText length]) : NSMakeRange(NSNotFound, 0);
  if (NS_KEYLOG)
...
- (NSRect)firstRectForCharacterRange:(NSRange)theRange {
  ...
  pt.y = WINDOW_TO_FRAME_PIXEL_Y(win, win->phys_cursor.y + FRAME_LINE_HEIGHT(emacsframe));
...
#if MAC_OS_X_VERSION_MIN_REQUIRED < 1070 || defined(NS_IMPL_GNUSTEP)
  {
...
#if defined(NS_IMPL_COCOA) || GNUSTEP_GUI_MAJOR_VERSION > 0 || GNUSTEP_GUI_MINOR_VERSION > 22
...
- (void)mouseDown:(NSEvent *)theEvent {
  ...
  if ([theEvent type] == NSEventTypeScrollWheel) {
    ...
    if ([theEvent respondsToSelector:@selector(hasPreciseScrollingDeltas)]) {
      ...
      if (!ns_use_mwheel_momentum && [theEvent momentumPhase] != NSEventPhaseNone)
        return;
      ...
      if ([theEvent hasPreciseScrollingDeltas]) {
        ...
        if (FIXNUMP(ns_mwheel_line_height))
          ...
          lineHeight = default_line_pixel_height(XWINDOW(FRAME_SELECTED_WINDOW(emacsframe)));
        }
        ...
        if (abs(totalDeltaX) > abs(totalDeltaY) && abs(totalDeltaX) > lineHeight) {
          horizontal = YES;
          ...
          totalDeltaY = 0;
        } else if (abs(totalDeltaY) >= abs(totalDeltaX) && abs(totalDeltaY) > lineHeight) {
          horizontal = NO;
        ...
        lines = (ns_use_mwheel_acceleration) ? ceil(fabs(delta)) : 1;
      ...
      emacs_event->code = 0;
      emacs_event->modifiers = EV_MODIFIERS(theEvent) | (scrollUp ? up_modifier : down_modifier);
#if MAC_OS_X_VERSION_MIN_REQUIRED < 1070
    } else
#endif
...
#if defined(NS_IMPL_GNUSTEP) || MAC_OS_X_VERSION_MIN_REQUIRED < 1070
    {
      ...
      emacs_event->code = 0;
      emacs_event->modifiers = EV_MODIFIERS(theEvent) | ((delta > 0) ? up_modifier : down_modifier);
    }
#endif
  } else {
    ...
    if (WINDOWP(emacsframe->tab_bar_window) &&
      ...
      if (tab_bar_p)
        tab_bar_arg =
            handle_tab_bar_click(emacsframe, x, y, EV_UDMODIFIERS(theEvent) & down_modifier,
    ...
    emacs_event->modifiers = EV_MODIFIERS(theEvent) | EV_UDMODIFIERS(theEvent);
  }
...
- (void)mouseMoved:(NSEvent *)e {
  ...
  if (!NILP(Vmouse_autoselect_window)) {
    ...
    Lisp_Object window = window_from_coordinates(emacsframe, pt.x, pt.y, 0, 0, 0);

    if (WINDOWP(window) && !EQ(window, last_mouse_window) && !EQ(window, selected_window) &&
        (!NILP(focus_follows_mouse) ||
         (EQ(XWINDOW(window)->frame, XWINDOW(selected_window)->frame)))) {
  ...
  if (!NILP(help_echo_string) || !NILP(previous_help_echo_string)) {
    ...
    gen_help_event(help_echo_string, frame, help_echo_window, help_echo_object, help_echo_pos);
  }
...
/* Normalize frame to gridded text size.  */
{
  ...
  NSTRACE("[EmacsView windowWillResize:toSize: " NSTRACE_FMT_SIZE "]", NSTRACE_ARG_SIZE(frameSize));
  ...
  if (fs_state == FULLSCREEN_MAXIMIZED &&
      (maximized_width != (int)frameSize.width || maximized_height != (int)frameSize.height))
    [self setFSValue:FULLSCREEN_NONE];
  else if (fs_state == FULLSCREEN_WIDTH && maximized_width != (int)frameSize.width)
    [self setFSValue:FULLSCREEN_NONE];
  else if (fs_state == FULLSCREEN_HEIGHT && maximized_height != (int)frameSize.height)
    [self setFSValue:FULLSCREEN_NONE];
  ...
  if (![self isFullscreen]) {
    extra = FRAME_NS_TITLEBAR_HEIGHT(emacsframe) + FRAME_TOOLBAR_HEIGHT(emacsframe);
  }
  ...
  rows = FRAME_PIXEL_HEIGHT_TO_TEXT_LINES(emacsframe, frameSize.height - extra);
  if (rows < MINHEIGHT)
    rows = MINHEIGHT;
#ifdef NS_IMPL_COCOA
  {
    ...
    } else if (fs_state == FULLSCREEN_NONE && !maximizing_resize && [[self window] title] != NULL) {
      char *size_title;
  ...
  if (!frame_resize_pixelwise) {
    ...
    if (r.size.height != frameSize.height) {
      frameSize.height = FRAME_TEXT_LINES_TO_PIXEL_HEIGHT(emacsframe, rows) + extra;
    }
  }
...
- (void)setFrame:(NSRect)frameRect {
  NSTRACE("[EmacsView setFrame:" NSTRACE_FMT_RECT "]", NSTRACE_ARG_RECT(frameRect));
...
- (instancetype)initFrameFromEmacs:(struct frame *)f {
  ...
  [self initWithFrame:NSMakeRect(0, 0, FRAME_TEXT_COLS_TO_PIXEL_WIDTH(f, f->text_cols),
  ...
  if ([self respondsToSelector:@selector(allocateGState)])
    ...
    [self allocateGState];
#endif
  [NSApp registerServicesMenuSendTypes:ns_send_types returnTypes:[NSArray array]];
...
- (BOOL)windowShouldZoom:(NSWindow *)sender toFrame:(NSRect)newFrame {
  NSTRACE(("[EmacsView windowShouldZoom:toFrame:" NSTRACE_FMT_RECT "]" NSTRACE_FMT_RETURN "YES"),
...
- (NSRect)windowWillUseStandardFrame:(NSWindow *)sender defaultFrame:(NSRect)defaultFrame {
  ...
  NSTRACE(("[EmacsView windowWillUseStandardFrame:defaultFrame:" NSTRACE_FMT_RECT "]"),
  ...
  if (fs_before_fs != -1) /* Entering fullscreen */
  {
    ...
    if (fs_state != FULLSCREEN_MAXIMIZED && fs_state != FULLSCREEN_WIDTH) {
      ns_userRect.size.width = result.size.width;
      ns_userRect.origin.x = result.origin.x;
    }

    if (fs_state != FULLSCREEN_MAXIMIZED && fs_state != FULLSCREEN_HEIGHT) {
    ...
    if (next_maximized == FULLSCREEN_HEIGHT ||
        (next_maximized == -1 && abs((int)(defaultFrame.size.height - result.size.height)) >
      ...
      [self setFSValue:FULLSCREEN_WIDTH];
               ...
               (next_maximized == -1 && abs((int)(defaultFrame.size.width - result.size.width)) >
...
- (BOOL)isFullscreen {
  ...
  NSTRACE("[EmacsView isFullscreen] " NSTRACE_FMT_RETURN " %d", (int)res);
...
- (void)toggleFullScreen:(id)sender {
  ...
  f = emacsframe;
  wr = [w frame];
  col = ns_lookup_indexed_color(NS_FACE_BACKGROUND(FACE_FROM_ID(f, DEFAULT_FACE_ID)), f);
  ...
  if (fs_state != FULLSCREEN_BOTH) {
    ...
    fw = [[EmacsWindow alloc] initWithEmacsFrame:emacsframe fullscreen:YES screen:screen];
    ...
    [self windowWillExitFullScreen];
    [fw setFrame:[[w contentView] frame] display:YES animate:ns_use_fullscreen_animation];
    [fw close];
...
- (void)setFSValue:(int)value {
  NSTRACE("[EmacsView setFSValue:" NSTRACE_FMT_FSTYPE "]", NSTRACE_ARG_FSTYPE(value));
...
- (void)mouseEntered:(NSEvent *)theEvent {
  ...
  if (emacsframe)
    FRAME_DISPLAY_INFO(emacsframe)->last_mouse_movement_time = EV_TIMESTAMP(theEvent);
}
...
- (instancetype)menuDown:sender {
  ...
  if (context_menu_value == -1)
    ...
    find_and_call_menu_selection(emacsframe, emacsframe->menu_bar_items_used,
                                 emacsframe->menu_bar_vector, (void *)tag);
  }
...
- (instancetype)toolbarClicked:(id)item {
  ...
  emacs_event->arg = AREF(emacsframe->tool_bar_items, idx + TOOL_BAR_ITEM_KEY);
  emacs_event->modifiers = EV_MODIFIERS(theEvent);
  EV_TRAILER(theEvent);
  return self;
}
...
{
  EmacsLayer *l = [[EmacsLayer alloc] initWithColorSpace:[[[self window] colorSpace] CGColorSpace]];
...
- (void)lockFocus {
  ...
  if ([self wantsLayer]) {
    ...
    [NSGraphicsContext
        setCurrentContext:[NSGraphicsContext graphicsContextWithCGContext:context flipped:YES]];
  }
...
- (void)copyRect:(NSRect)srcRect to:(NSPoint)dest {
  ...
  NSRect dstRect = NSMakeRect(dest.x, dest.y, NSWidth(srcRect), NSHeight(srcRect));
  NSRect frame = [self frame];

  /* TODO: This check is an attempt to debug a rare graphical glitch
     on macOS and should be removed before the Emacs 28 release.  */
  if (!NSContainsRect(frame, srcRect) || !NSContainsRect(frame, dstRect)) {
    NSLog(@"[EmacsView copyRect:to:] Attempting to copy to or "
           "from an area outside the graphics buffer.");
    NSLog(@"  Frame: (%f, %f) %f×%f", NSMinX(frame), NSMinY(frame), NSWidth(frame),
          NSHeight(frame));
    NSLog(@"  Source: (%f, %f) %f×%f", NSMinX(srcRect), NSMinY(srcRect), NSWidth(srcRect),
          NSHeight(srcRect));
    NSLog(@"  Destination: (%f, %f) %f×%f", NSMinX(dstRect), NSMinY(dstRect), NSWidth(dstRect),
          NSHeight(dstRect));
  }
  ...
  if ([self wantsLayer]) {
    ...
    void *srcPixels =
        (char *)pixels + (int)(NSMinY(srcRect) * scale * rowSize + NSMinX(srcRect) * scale * bpp);
    void *dstPixels = (char *)pixels + (int)(dest.y * scale * rowSize + dest.x * scale * bpp);

    if (NSIntersectsRect(srcRect, dstRect) && NSMinY(srcRect) < NSMinY(dstRect))
      for (int y = NSHeight(srcRect) * scale - 1; y >= 0; y--)
        memmove((char *)dstPixels + y * rowSize, (char *)srcPixels + y * rowSize, srcRowSize);
    else
...
- (void)drawRect:(NSRect)rect {
  NSTRACE("[EmacsView drawRect:" NSTRACE_FMT_RECT "]", NSTRACE_ARG_RECT(rect));
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
  ...
  if (type == 0) {
    ...
    NSArray *urls = [pb readObjectsForClasses:@[ [NSURL self] ] options:nil];
  ...
  emacs_event->arg = Fcons(type_sym, Fcons(operations, strings));
  EV_TRAILER(theEvent);
...
- (id)validRequestorForSendType:(NSString *)typeSent returnType:(NSString *)typeReturned {
  ...
  return [super validRequestorForSendType:typeSent returnType:typeReturned];
}
...
- (instancetype)setMiniwindowImage:(BOOL)setMini {
  ...
  if (image != emacsframe->output_data.ns->miniimage) {
    if (image && [image isKindOfClass:[EmacsImage class]])
      [image release];
    [[self window] setMiniwindowImage:setMini ? emacsframe->output_data.ns->miniimage : nil];
  }
...
- (instancetype)initWithEmacsFrame:(struct frame *)f
  ...
  self =
      [super initWithContentRect:NSMakeRect(0, 0, FRAME_TEXT_COLS_TO_PIXEL_WIDTH(f, f->text_cols),
                                            FRAME_TEXT_LINES_TO_PIXEL_HEIGHT(f, f->text_lines))
                       styleMask:styleMask
  ...
  if (self) {
    ...
    if (!NILP(f->icon_name))
      [self setMiniwindowTitle:[NSString stringWithLispString:f->icon_name]];
    ...
    if (screen != 0) {
      NSPoint pt =
          NSMakePoint(IN_BOUND(-SCREENMAX, f->left_pos + NS_PARENT_WINDOW_LEFT_POS(f), SCREENMAX),
                      IN_BOUND(-SCREENMAX, NS_PARENT_WINDOW_TOP_POS(f) - f->top_pos, SCREENMAX));
...
- (void)createToolbar:(struct frame *)f {
  ...
  EmacsToolbar *toolbar =
      [[EmacsToolbar alloc] initForView:view
...
static NSInteger nswindow_orderedIndex_sort(id w1, id w2, void *c) {
...
- (void)orderBack:(id)sender {
  ...
  if (parent) {
    NSArray *children =
        [[parent childWindows] sortedArrayUsingFunction:nswindow_orderedIndex_sort context:nil];
    [parent removeChildWindow:self];
    [parent addChildWindow:self ordered:NSWindowAbove];
...
- (BOOL)restackWindow:(NSWindow *)win above:(BOOL)above {
  ...
  if ([self parentWindow] != [win parentWindow])
    ...
    [self orderWindow:(above ? NSWindowAbove : NSWindowBelow) relativeTo:[win windowNumber]];
  else {
    NSInteger index;
    NSWindow *parent = [self parentWindow];
...
- (id)accessibilityAttributeValue:(NSString *)attribute {
  ...
  if ([attribute isEqualToString:NSAccessibilitySelectedTextAttribute] && curbuf &&
      !NILP(BVAR(curbuf, mark_active))) {
...
- (NSRect)constrainFrameRect:(NSRect)frameRect toScreen:(NSScreen *)screen {
  ...
  return constrain_frame_rect(frameRect, [(EmacsView *)[self delegate] isFullscreen]);
}
...
- (void)zoom:(id)sender {
  ...
  if (screen != nil) {
    ...
    struct EmacsMargins margins = ns_screen_margins_ignoring_hidden_dock(screen);
    ...
    if (newWr.size.width != wr.size.width || newWr.size.height != wr.size.height ||
        newWr.origin.x != wr.origin.x || newWr.origin.y != wr.origin.y) {
  ...
  if (screen != nil) {
    ...
    sr = [[self delegate] windowWillUseStandardFrame:self defaultFrame:sr];
...
- (void)setFrame:(NSRect)windowFrame display:(BOOL)displayViews {
  NSTRACE("[EmacsWindow setFrame:" NSTRACE_FMT_RECT " display:%d]", NSTRACE_ARG_RECT(windowFrame),
          displayViews);
...
- (void)setFrame:(NSRect)windowFrame display:(BOOL)displayViews animate:(BOOL)performAnimation {
  NSTRACE("[EmacsWindow setFrame:" NSTRACE_FMT_RECT " display:%d performAnimation:%d]",
...
- (void)setFrameTopLeftPoint:(NSPoint)point {
  NSTRACE("[EmacsWindow setFrameTopLeftPoint:" NSTRACE_FMT_POINT "]", NSTRACE_ARG_POINT(point));
...
#if defined(NS_IMPL_GNUSTEP) || MAC_OS_X_VERSION_MIN_REQUIRED < 101300
#if MAC_OS_X_VERSION_MAX_ALLOWED >= 101300
  if ([[NSCursor arrowCursor] respondsToSelector:@selector(setOnMouseEntered)])
#endif
...
- (instancetype)repeatScroll:(NSTimer *)scrollEntry {
  ...
  if (inKnob || [scroll_repeat_entry timeInterval] == SCROLL_BAR_FIRST_DELAY) {
    ...
    scroll_repeat_entry = [[NSTimer scheduledTimerWithTimeInterval:SCROLL_BAR_CONTINUOUS_DELAY
                                                            target:self
...
- (void)mouseDown:(NSEvent *)e {
  ...
  case NSScrollerDecrementPage:
    last_hit_part = horizontal ? scroll_bar_before_handle : scroll_bar_above_handle;
    break;
  case NSScrollerIncrementPage:
    last_hit_part = horizontal ? scroll_bar_after_handle : scroll_bar_below_handle;
    break;
#if defined(NS_IMPL_GNUSTEP) || MAC_OS_X_VERSION_MIN_REQUIRED < 1070
  case NSScrollerDecrementLine:
    last_hit_part = horizontal ? scroll_bar_left_arrow : scroll_bar_up_arrow;
    break;
  case NSScrollerIncrementLine:
    last_hit_part = horizontal ? scroll_bar_right_arrow : scroll_bar_down_arrow;
    break;
#endif
  case NSScrollerKnob:
    last_hit_part = horizontal ? scroll_bar_horizontal_handle : scroll_bar_handle;
    break;
  case NSScrollerKnobSlot: /* GNUstep-only */
    last_hit_part = scroll_bar_move_ratio;
    break;
  ...
  if (part == NSScrollerKnob || part == NSScrollerKnobSlot) {
    ...
    sr = [self convertRect:[self rectForPart:NSScrollerKnobSlot] toView:nil];
    if (horizontal) {
      length = NSWidth(sr);
    ...
    if (edge)
      ...
      kr = [self convertRect:[self rectForPart:NSScrollerKnob] toView:nil];
      if (horizontal)
    ...
    scroll_repeat_entry = [[NSTimer scheduledTimerWithTimeInterval:SCROLL_BAR_FIRST_DELAY
                                                            target:self
                                                          selector:@selector(repeatScroll:)
                                                          userInfo:0
                                                           repeats:YES] retain];
...
- (BOOL)checkDimensions {
  ...
  IOSurfaceRef s = currentSurface ? currentSurface : (IOSurfaceRef)[cache firstObject];

  return !s || (IOSurfaceGetWidth(s) == width && IOSurfaceGetHeight(s) == height);
}
...
- (CGContextRef)getContext {
  ...
  if (!context) {
    ...
    if (!surface && [cache count] >= CACHE_MAX_SIZE) {
      ...
      int bytesPerRow = IOSurfaceAlignProperty(kIOSurfaceBytesPerRow, width * 4);

      surface = IOSurfaceCreate((CFDictionaryRef) @{
        (id)kIOSurfaceWidth : [NSNumber numberWithInt:width],
    ...
    context = CGBitmapContextCreate(
        IOSurfaceGetBaseAddress(currentSurface), IOSurfaceGetWidth(currentSurface),
        IOSurfaceGetHeight(currentSurface), 8, IOSurfaceGetBytesPerRow(currentSurface), colorSpace,
...
- (void)display {
  ...
  if (context) {
    ...
    [self performSelectorOnMainThread:@selector(getContext) withObject:nil waitUntilDone:NO];
  }
}
...
static Lisp_Object ns_new_font(struct frame *f, Lisp_Object font_object, int fontset) {
  ...
  if (FRAME_CONFIG_SCROLL_BAR_WIDTH(f) > 0) {
    ...
    FRAME_CONFIG_SCROLL_BAR_COLS(f) = (FRAME_CONFIG_SCROLL_BAR_WIDTH(f) + wid - 1) / wid;
  } else {
    int wid = FRAME_COLUMN_WIDTH(f);
  ...
  if (FRAME_CONFIG_SCROLL_BAR_HEIGHT(f) > 0) {
    ...
    FRAME_CONFIG_SCROLL_BAR_LINES(f) = (FRAME_CONFIG_SCROLL_BAR_HEIGHT(f) + height - 1) / height;
  } else {
    int height = FRAME_LINE_HEIGHT(f);
...
const char *ns_xlfd_to_fontname(const char *xlfd)
/* --------------------------------------------------------------------------
...
void syms_of_nsterm(void) {
  ...
  DEFVAR_LISP("ns-input-file", ns_input_file, "The file specified in the last NS event.");
  ns_input_file = Qnil;
  ...
  DEFVAR_LISP("ns-input-font", ns_input_font, "The font specified in the last NS event.");
  ns_input_font = Qnil;
  ...
  DEFVAR_LISP("ns-input-line", ns_input_line, "The line specified in the last NS event.");
  ns_input_line = Qnil;
...
This variable is ignored on macOS < 10.7 and GNUstep.  Default is t.  */);
  ...
  DEFVAR_BOOL("x-use-underline-position-properties", x_use_underline_position_properties, doc
              : /* SKIP: real doc in xterm.c.  */);
  x_use_underline_position_properties = 0;
  DEFSYM(Qx_use_underline_position_properties, "x-use-underline-position-properties");

  DEFVAR_BOOL("x-underline-at-descent-line", x_underline_at_descent_line, doc
              : /* SKIP: real doc in xterm.c.  */);
