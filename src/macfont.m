...
static CTLineRef mac_font_create_line_with_string_and_font(CFStringRef, CTFontRef);
static Boolean mac_font_descriptor_supports_languages(CTFontDescriptorRef, CFArrayRef);
static CFStringRef mac_font_create_preferred_family_for_attributes(CFDictionaryRef);
static CFIndex mac_font_shape(CTFontRef, CFStringRef, struct mac_glyph_layout *, CFIndex,
                              enum lgstring_direction);
static CFArrayRef mac_font_copy_default_descriptors_for_language(CFStringRef);
static CFStringRef mac_font_copy_default_name_for_charset_and_languages(CFCharacterSetRef,
                                                                        CFArrayRef);
#if USE_CT_GLYPH_INFO
static CGGlyph mac_ctfont_get_glyph_for_cid(CTFontRef, CTCharacterCollection, CGFontIndex);
#endif
...
static Boolean cfnumber_get_font_symbolic_traits_value(CFNumberRef, CTFontSymbolicTraits *);
static void macfont_store_descriptor_attributes(CTFontDescriptorRef, Lisp_Object);
...
static int macfont_glyph_extents(struct font *, CGGlyph, struct font_metrics *, CGFloat *, int);
static CFMutableDictionaryRef macfont_create_attributes_with_spec(Lisp_Object);
static Boolean macfont_supports_charset_and_languages_p(CTFontDescriptorRef, CFCharacterSetRef,
                                                        Lisp_Object, CFArrayRef);
static Boolean macfont_closest_traits_index_p(CFArrayRef, CTFontSymbolicTraits, CFIndex);
static CFDataRef mac_font_copy_uvs_table(CTFontRef);
static void mac_font_get_glyphs_for_variants(CFDataRef, UTF32Char, const UTF32Char[], CGGlyph[],
                                             CFIndex);

static void clear_bg_rect(NSColor *clearColor, CGContextRef context, CGRect *background_rect) {
  do {
...
static Lisp_Object cfdata_to_lisp(CFDataRef data) {
...
static Lisp_Object cfstring_to_lisp_nodecode(CFStringRef string) {
  ...
  data = CFStringCreateExternalRepresentation(NULL, string, kCFStringEncodingUTF8, '?');
  if (data) {
...
static CFStringRef cfstring_create_with_string_noencode(Lisp_Object s) {
  CFStringRef string =
      CFStringCreateWithBytes(NULL, SDATA(s), SBYTES(s), kCFStringEncodingUTF8, false);

  if (string == NULL)
    /* Failed to interpret as UTF 8.  Fall back on Mac Roman.  */
    string = CFStringCreateWithBytes(NULL, SDATA(s), SBYTES(s), kCFStringEncodingMacRoman, false);

  return string;
}

static CFIndex mac_font_get_weight(CTFontRef font) {
  NSFont *nsFont = (NSFont *)font;

  return [[NSFontManager sharedFontManager] weightOfFont:nsFont];
}

static CGFloat mac_screen_font_get_advance_width_for_glyph(ScreenFontRef font, CGGlyph glyph) {
...
#if !USE_CT_GLYPH_INFO
static CGGlyph mac_font_get_glyph_for_cid(CTFontRef font, NSCharacterCollection collection,
                                          CGFontIndex cid) {
  CGGlyph result = kCGFontIndexInvalid;
  NSFont *nsFont = (NSFont *)font;
  unichar characters[] = {0xfffd};
  NSString *string = [NSString stringWithCharacters:characters length:ARRAYELTS(characters)];
  NSGlyphInfo *glyphInfo =
      [NSGlyphInfo glyphInfoWithCharacterIdentifier:cid collection:collection baseString:string];
  NSDictionary *attributes =
      [NSDictionary dictionaryWithObjectsAndKeys:nsFont, NSFontAttributeName, glyphInfo,
                                                 NSGlyphInfoAttributeName, nil];
  NSTextStorage *textStorage = [[NSTextStorage alloc] initWithString:string attributes:attributes];
  ...
  fontInTextStorage = [textStorage attribute:NSFontAttributeName atIndex:0 effectiveRange:NULL];
...
static ScreenFontRef mac_screen_font_create_with_name(CFStringRef name, CGFloat size) {
  NSFont *result, *font;
...
static Boolean mac_screen_font_get_metrics(ScreenFontRef font, CGFloat *ascent, CGFloat *descent,
                                           CGFloat *leading) {
  ...
  usedRect = [layoutManager lineFragmentUsedRectForGlyphAtIndex:0 effectiveRange:NULL];
...
static CFIndex mac_font_shape_1(NSFont *font, NSString *string,
  ...
  [textStorage
      appendAttributedString:([[[NSAttributedString alloc] initWithString:@" "] autorelease])];
  ...
  while (i < stringLength) {
    NSRange range;
    NSFont *fontInTextStorage = [textStorage attribute:NSFontAttributeName
                                               atIndex:i
...
#define RIGHT_TO_LEFT_P permutation
    ...
    for (range = NSMakeRange(0, 0); NSMaxRange(range) < used; range.length++) {
      ...
      if (characterIndex >= NSMaxRange(compRange)) {
        ...
        do {
          ...
          compRange.length = NSMaxRange(characterRange) - compRange.location;
    ...
    if (!RIGHT_TO_LEFT_P)
      ...
      NSUInteger nrects;
      NSRect *glyphRects = [layoutManager
            rectArrayForGlyphRange:(NSMakeRange(0, numberOfGlyphs))
          withinSelectedGlyphRange:(NSMakeRange(NSNotFound, 0))inTextContainer:textContainer
                         rectCount:&nrects];
    ...
    for (i = 0; i < used; i++) {
      ...
      for (nextGlyphIndex = glyphIndex + 1; nextGlyphIndex < numberOfGlyphs; nextGlyphIndex++)
        if (![layoutManager notShownAttributeForGlyphAtIndex:nextGlyphIndex])
          break;
      ...
      if (!RIGHT_TO_LEFT_P) {
        ...
        if (prevGlyphIndex == 0)
          ...
          glyphRange = NSMakeRange(glyphIndex, nextGlyphIndex - glyphIndex);
        ...
        if (nextGlyphIndex == numberOfGlyphs)
          glyphRange = NSMakeRange(prevGlyphIndex, numberOfGlyphs - prevGlyphIndex);
        else
          glyphRange = NSMakeRange(prevGlyphIndex, glyphIndex + 1 - prevGlyphIndex);
        glyphRects = [layoutManager
              rectArrayForGlyphRange:glyphRange
            withinSelectedGlyphRange:(NSMakeRange(NSNotFound, 0))inTextContainer:textContainer
                           rectCount:&nrects];
...
static CFIndex mac_screen_font_shape(ScreenFontRef font, CFStringRef string,
                                     struct mac_glyph_layout *glyph_layouts, CFIndex glyph_len,
                                     enum lgstring_direction dir) {
  return mac_font_shape_1([(NSFont *)font printerFont], (NSString *)string, glyph_layouts,
                          glyph_len, dir);
}

static CGColorRef get_cgcolor(unsigned long idx, struct frame *f) {
...
} cf_charset_table[] = {{"iso8859-1", {0x00A0, 0x00A1, 0x00B4, 0x00BC, 0x00D0}},
...
} macfont_language_default_font_names[] = {{CFSTR("ja"),
                                            {CFSTR("HiraKakuProN-W3"), /* 10.5 - 10.9 */
                                             CFSTR("HiraKakuPro-W3"),  /* 10.4 */
                                             NULL}},
                                           {CFSTR("ko"),
                                            {CFSTR("AppleSDGothicNeo-Regular"), /* 10.8 - 10.9 */
                                             CFSTR("AppleGothic"),              /* 10.4 - 10.7 */
                                             NULL}},
                                           {CFSTR("zh-Hans"),
                                            {CFSTR("STHeitiSC-Light"), /* 10.6 - 10.9 */
                                             CFSTR("STXihei"),         /* 10.4 - 10.5 */
                                             NULL}},
                                           {CFSTR("zh-Hant"),
                                            {CFSTR("STHeitiTC-Light"), /* 10.6 - 10.9 */
...
void macfont_update_antialias_threshold(void) {
  int threshold;
  Boolean valid_p;

  threshold = CFPreferencesGetAppIntegerValue(CFSTR("AppleAntiAliasingThreshold"),
                                              kCFPreferencesCurrentApplication, &valid_p);
  if (valid_p)
    macfont_antialias_threshold = threshold;
}

static inline Lisp_Object macfont_intern_prop_cfstring(CFStringRef cfstring) {
  Lisp_Object string = cfstring_to_lisp_nodecode(cfstring);

  return font_intern_prop(SSDATA(string), SBYTES(string), 1);
}

static inline CFIndex macfont_store_utf32char_to_unichars(UTF32Char c, UniChar *unichars) {
  if (c < 0x10000) {
    unichars[0] = c;
...
static Boolean cfnumber_get_font_symbolic_traits_value(CFNumberRef number,
...
static CGFloat mac_font_descriptor_get_adjusted_weight(CTFontDescriptorRef desc, CGFloat val) {
...
static void macfont_store_descriptor_attributes(CTFontDescriptorRef desc,
  ...
  if (str) {
    ASET(spec_or_entity, FONT_FAMILY_INDEX, macfont_intern_prop_cfstring(str));
    CFRelease(str);
  }
  ...
  if (dict) {
    ...
    } numeric_traits[] = {{FONT_WEIGHT_INDEX,
                           kCTFontWeightTrait,
                           {{-0.4, 50},    /* light */
    ...
    for (i = 0; i < ARRAYELTS(numeric_traits); i++) {
      ...
      if (num && CFNumberGetValue(num, kCFNumberCGFloatType, &floatval)) {
        ...
        floatval = (point - 1)->y + ((floatval - (point - 1)->x) *
                                     ((point->y - (point - 1)->y) / (point->x - (point - 1)->x)));
        FONT_SET_STYLE(spec_or_entity, numeric_traits[i].index, make_fixnum(lround(floatval)));
      }
    }
    ...
    if (num) {
      ...
      spacing =
          (sym_traits & kCTFontTraitMonoSpace ? FONT_SPACING_MONO : FONT_SPACING_PROPORTIONAL);
...
static Lisp_Object macfont_descriptor_entity(CTFontDescriptorRef desc, Lisp_Object extra,
  ...
  font_put_extra(entity, QCfont_entity,
                 Fcons(make_mint_ptr((void *)name), make_fixnum(sym_traits)));
  if (synth_sym_traits & kCTFontTraitItalic)
    FONT_SET_STYLE(entity, FONT_SLANT_INDEX, make_fixnum(FONT_SLANT_SYNTHETIC_ITALIC));
  if (synth_sym_traits & kCTFontTraitBold)
    FONT_SET_STYLE(entity, FONT_WEIGHT_INDEX, make_fixnum(FONT_WEIGHT_SYNTHETIC_BOLD));
  if (synth_sym_traits & kCTFontTraitMonoSpace)
    ASET(entity, FONT_SPACING_INDEX, make_fixnum(FONT_SPACING_SYNTHETIC_MONO));
...
static void macfont_invalidate_family_cache(void) {
...
static bool macfont_get_family_cache_if_present(Lisp_Object symbol, CFStringRef *string) {
  if (HASH_TABLE_P(macfont_family_cache)) {
    struct Lisp_Hash_Table *h = XHASH_TABLE(macfont_family_cache);
...
static void macfont_set_family_cache(Lisp_Object symbol, CFStringRef string) {
...
static void macfont_invalidate_available_families_cache(void) {
...
static void macfont_handle_font_change_notification(CFNotificationCenterRef center, void *observer,
...
static void macfont_init_font_change_handler(void) {
...
static CFArrayRef macfont_copy_available_families_cache(void) {
  ...
  return (macfont_available_families_cache ? CFRetain(macfont_available_families_cache) : NULL);
}

static CFStringRef macfont_create_family_with_symbol(Lisp_Object symbol) {
  ...
  if (family_name) {
    attributes = CFDictionaryCreate(NULL, (const void **)&kCTFontFamilyNameAttribute,
                                    (const void **)&family_name, 1, &kCFTypeDictionaryKeyCallBacks,
  ...
  if (pat_desc) {
    CTFontDescriptorRef desc = CTFontDescriptorCreateMatchingFontDescriptor(pat_desc, NULL);

    if (desc) {
      result = CTFontDescriptorCopyAttribute(desc, kCTFontFamilyNameAttribute);
      CFRelease(desc);
    }
...
#define METRICS_VALUE(metrics, member)                                                             \
  ((int)(((unsigned int)(metrics)->member##_high << 8) | (metrics)->member##_low))
#define METRICS_SET_VALUE(metrics, member, value)                                                  \
  do {                                                                                             \
    short tmp = (value);                                                                           \
    (metrics)->member##_low = tmp & 0xff;                                                          \
    (metrics)->member##_high = tmp >> 8;                                                           \
  } while (0)
...
#define METRICS_STATUS(metrics) (METRICS_VALUE(metrics, ascent) + METRICS_VALUE(metrics, descent))
#define METRICS_SET_STATUS(metrics, status)                                                        \
  do {                                                                                             \
    METRICS_SET_VALUE(metrics, ascent, 0);                                                         \
    METRICS_SET_VALUE(metrics, descent, status);                                                   \
  } while (0)
...
static int macfont_monospace_width_multiplier(struct font *font, CGFloat width) {
  struct macfont_info *macfont_info = (struct macfont_info *)font;
  int multiplier = 0;

  if (macfont_info->spacing == MACFONT_SPACING_MONO && font->space_width != 0) {
    multiplier = lround(width / font->space_width);
    if (multiplier == 1 || lround(width / multiplier) != font->space_width)
      multiplier = 0;
  }
...
static int macfont_glyph_extents(struct font *font, CGGlyph glyph, struct font_metrics *metrics,
                                 CGFloat *advance_delta, int force_integral_p) {
  struct macfont_info *macfont_info = (struct macfont_info *)font;
  ...
  if (row >= macfont_info->metrics_nrows) {
    macfont_info->metrics =
        xrealloc(macfont_info->metrics, sizeof(struct macfont_metrics *) * (row + 1));
  ...
  if (METRICS_STATUS(cache) == METRICS_INVALID) {
    ...
    if (macfont_info->spacing == MACFONT_SPACING_MONO) {
      ...
      if (multiplier) {
        ...
        cache->width_frac = lround((fwidth / multiplier - font->space_width) * WIDTH_FRAC_SCALE);
      } else {
      ...
      cache->width_int = lround(fwidth);
      cache->width_frac = lround((fwidth - cache->width_int) * WIDTH_FRAC_SCALE);
    }
  ...
  if (metrics) {
    if (METRICS_STATUS(cache) == METRICS_WIDTH_VALID) {
      ...
      if (macfont_info->synthetic_italic_p) {
        ...
        bounds.origin = CGPointApplyAffineTransform(bounds.origin, synthetic_italic_atfm);
        bounds.size = CGSizeApplyAffineTransform(bounds.size, synthetic_italic_atfm);
      }
      ...
      case MACFONT_SPACING_PROPORTIONAL:
        bounds.origin.x += -(cache->width_frac / (CGFloat)(WIDTH_FRAC_SCALE * 2));
        break;
      ...
      case MACFONT_SPACING_SYNTHETIC_MONO:
        bounds.origin.x += (cache->width_int + (cache->width_frac / (CGFloat)WIDTH_FRAC_SCALE));
        break;
      }
      if (bounds.size.width > 0) {
        bounds.origin.x -= LCD_FONT_SMOOTHING_LEFT_MARGIN;
        bounds.size.width += (LCD_FONT_SMOOTHING_LEFT_MARGIN + LCD_FONT_SMOOTHING_RIGHT_MARGIN);
      }
  ...
  if (advance_delta) {
    ...
    case MACFONT_SPACING_PROPORTIONAL:
      *advance_delta =
          (force_integral_p ? 0 : -(cache->width_frac / (CGFloat)(WIDTH_FRAC_SCALE * 2)));
      break;
    case MACFONT_SPACING_MONO:
      if (cache->width_frac)
        *advance_delta = 0;
      else {
        CGFloat delta = -((cache->width_frac / (CGFloat)(WIDTH_FRAC_SCALE * 2)) *
                          (cache->width_int / font->space_width));
    ...
    case MACFONT_SPACING_SYNTHETIC_MONO:
      *advance_delta =
          (force_integral_p ? cache->width_int
                            : (cache->width_int + (cache->width_frac / (CGFloat)WIDTH_FRAC_SCALE)));
      break;
    }
  }
...
static CGGlyph macfont_get_glyph_for_cid(struct font *font, NSCharacterCollection, CGFontIndex);
static CFDataRef macfont_get_uvs_table(struct font *, NSCharacterCollection *);

static struct macfont_cache *macfont_lookup_cache(CFStringRef key) {
  struct macfont_cache *cache;
  ...
  if (macfont_cache_dictionary == NULL) {
    ...
    cache = ((struct macfont_cache *)CFDictionaryGetValue(macfont_cache_dictionary, key));
  ...
  if (cache == NULL) {
    ...
    if (macfont) {
      ...
      if (CFEqual(key, CFSTR("LastResort"))) {
        ...
        cache->cf_charset = CFCharacterSetCreateWithCharactersInRange(NULL, range);
      }
      if (cache->cf_charset == NULL)
        cache->cf_charset = CTFontCopyCharacterSet(macfont);
      CFDictionaryAddValue(macfont_cache_dictionary, key, (const void *)cache);
      CFRelease(macfont);
    }
  }
...
static struct macfont_cache *macfont_retain_cache(struct macfont_cache *cache) {
  cache->reference_count++;

  return cache;
}

static void macfont_release_cache(struct macfont_cache *cache) {
...
static CFCharacterSetRef macfont_get_cf_charset(struct font *font) {
  struct macfont_info *macfont_info = (struct macfont_info *)font;

  return macfont_info->cache->cf_charset;
}

static CFCharacterSetRef macfont_get_cf_charset_for_name(CFStringRef name) {
  struct macfont_cache *cache = macfont_lookup_cache(name);

  return cache->cf_charset;
}

static CGGlyph macfont_get_glyph_for_character(struct font *font, UTF32Char c) {
  struct macfont_info *macfont_info = (struct macfont_info *)font;
  CTFontRef macfont = macfont_info->macfont;
  ...
  if (c < 0xD800 || (c > 0xDFFF && c < 0x10000)) {
    ...
    if (nkeys_or_perm < ROW_PERM_OFFSET) {
      ...
      if (row != 0) {
        ...
        if (cache->glyph.dictionary == NULL)
          cache->glyph.dictionary = CFDictionaryCreateMutable(NULL, 0, NULL, NULL);
        dictionary = cache->glyph.dictionary;
        key = c / NGLYPHS_IN_VALUE;
        nshifts = ((c % NGLYPHS_IN_VALUE) * sizeof(CGGlyph) * 8);
        value = ((uintptr_t)CFDictionaryGetValue(dictionary, (const void *)key));
        glyph = (value >> nshifts);
        if (glyph)
          return glyph;
        ...
        if (nkeys_or_perm + 1 != ROW_PERM_OFFSET) {
          ch = c;
          if (!CTFontGetGlyphsForCharacters(macfont, &ch, &glyph, 1) || glyph == 0)
            glyph = kCGFontIndexInvalid;

          if (value == 0)
            cache->glyph.row_nkeys_or_perm[row] = nkeys_or_perm + 1;
          value |= ((uintptr_t)glyph << nshifts);
          CFDictionarySetValue(dictionary, (const void *)key, (const void *)value);
        ...
        for (key = row * (256 / NGLYPHS_IN_VALUE);; key++)
          if (CFDictionaryContainsKey(dictionary, (const void *)key)) {
            CFDictionaryRemoveValue(dictionary, (const void *)key);
            if (--nkeys == 0)
              break;
          }
      }
      ...
      nrows++;
      cache->glyph.matrix = xrealloc(cache->glyph.matrix, sizeof(CGGlyph *) * nrows);
    ...
    key = c / NGLYPHS_IN_VALUE;
    nshifts = ((c % NGLYPHS_IN_VALUE) * sizeof(CGGlyph) * 8);
    value = (uintptr_t)CFDictionaryGetValue(cache->glyph.dictionary, (const void *)key);
    glyph = (value >> nshifts);
    if (glyph == 0) {
      UniChar unichars[2];
      CGGlyph glyphs[2];
      ...
      value |= ((uintptr_t)glyph << nshifts);
      CFDictionarySetValue(cache->glyph.dictionary, (const void *)key, (const void *)value);
    }
...
static CGGlyph macfont_get_glyph_for_cid(struct font *font, NSCharacterCollection collection,
                                         CGFontIndex cid) {
  struct macfont_info *macfont_info = (struct macfont_info *)font;
  CTFontRef macfont = macfont_info->macfont;
...
static CFDataRef macfont_get_uvs_table(struct font *font, NSCharacterCollection *collection) {
  struct macfont_info *macfont_info = (struct macfont_info *)font;
  CTFontRef macfont = macfont_info->macfont;
  struct macfont_cache *cache = macfont_info->cache;
  ...
  if (cache->uvs.table == NULL) {
    ...
    NSCharacterCollection uvs_collection = NSIdentityMappingCharacterCollection;

    if (uvs_table == NULL && mac_font_get_glyph_for_cid(macfont, NSAdobeJapan1CharacterCollection,
      ...
      if (mac_uvs_table_adobe_japan1 == NULL)
        mac_uvs_table_adobe_japan1 =
            CFDataCreateWithBytesNoCopy(NULL, mac_uvs_table_adobe_japan1_bytes,
                                        sizeof(mac_uvs_table_adobe_japan1_bytes), kCFAllocatorNull);
...
static void macfont_text_extents(struct font *, const unsigned int *, int, struct font_metrics *);
static int macfont_draw(struct glyph_string *, int, int, int, int, bool);
static Lisp_Object macfont_shape(Lisp_Object, Lisp_Object);
static int macfont_variation_glyphs(struct font *, int c, unsigned variations[256]);
...
static Lisp_Object macfont_get_cache(struct frame *f) {
  Display_Info *dpyinfo = FRAME_DISPLAY_INFO(f);

  return (dpyinfo->name_list_element);
}

static int macfont_get_charset(Lisp_Object registry) {
  ...
  for (i = 0; cf_charset_table[i].name; i++)
    if (fast_c_string_match_ignore_case(regexp, cf_charset_table[i].name,
  ...
  if (!cf_charset_table[i].cf_charset) {
    ...
    for (j = 0; uniquifier[j]; j++) {
      count += macfont_store_utf32char_to_unichars(uniquifier[j], unichars + count);
      CFCharacterSetAddCharactersInRange(charset, CFRangeMake(uniquifier[j], 1));
    }
    ...
    cf_charset_table[i].cf_charset = CFCharacterSetCreateCopy(NULL, charset);
    CFRelease(charset);
...
static struct OpenTypeSpec *macfont_get_open_type_spec(Lisp_Object otf_spec) {
  ...
  for (i = 0; i < 2 && !NILP(otf_spec); i++, otf_spec = XCDR(otf_spec)) {
    ...
    spec->features[i] =
        (min(PTRDIFF_MAX, SIZE_MAX) / sizeof(int) < len ? 0
...
static CFMutableDictionaryRef macfont_create_attributes_with_spec(Lisp_Object spec) {
    ...
    CGPoint points[6];
      ...
      {FONT_WEIGHT_INDEX,
       ...
       {{-0.4, 50},    /* light */
        ...
        {0.4, 200},    /* bold */
        {CGFLOAT_MAX, CGFLOAT_MAX}}},
      {FONT_SLANT_INDEX, kCTFontSlantTrait, {{0, 100}, {0.1, 200}, {CGFLOAT_MAX, CGFLOAT_MAX}}},
      {FONT_WIDTH_INDEX, kCTFontWidthTrait, {{0, 100}, {1, 200}, {CGFLOAT_MAX, CGFLOAT_MAX}}}};

  registry = AREF(spec, FONT_REGISTRY_INDEX);
  if (NILP(registry) || EQ(registry, Qascii_0) || EQ(registry, Qiso10646_1) ||
  ...
  for (extra = AREF(spec, FONT_EXTRA_INDEX); CONSP(extra); extra = XCDR(extra)) {
    Lisp_Object key, val;
    ...
    if (EQ(key, QClang)) {
      ...
      for (; CONSP(val); val = XCDR(val))
        if (SYMBOLP(XCAR(val))) {
          CFStringRef lang = cfstring_create_with_string_noencode(SYMBOL_NAME(XCAR(val)));
    ...
    if (CONSP(chars) && CONSP(CDR(chars))) {
      ...
      for (chars = XCDR(chars); CONSP(chars); chars = XCDR(chars))
        if (CHARACTERP(XCAR(chars))) {
          UniChar unichars[2];
          CFIndex count = macfont_store_utf32char_to_unichars(XFIXNAT(XCAR(chars)), unichars);
  ...
  attributes = CFDictionaryCreateMutable(NULL, 0, &kCFTypeDictionaryKeyCallBacks,
                                         &kCFTypeDictionaryValueCallBacks);
  if (!attributes)
    goto err;
  ...
  if (SYMBOLP(tmp) && !NILP(tmp)) {
    ...
    if (!family)
      goto err;
    CFDictionaryAddValue(attributes, kCTFontFamilyNameAttribute, family);
    CFRelease(family);
  }

  traits = CFDictionaryCreateMutable(NULL, 4, &kCFTypeDictionaryKeyCallBacks,
                                     &kCFTypeDictionaryValueCallBacks);
  if (!traits)
    goto err;
  ...
  if (charset)
    CFDictionaryAddValue(attributes, kCTFontCharacterSetAttribute, charset);
  if (charset_string)
    CFDictionaryAddValue(attributes, MAC_FONT_CHARACTER_SET_STRING_ATTRIBUTE, charset_string);
  if (langarray)
...
finish:
  if (langarray)
    CFRelease(langarray);
  if (charset && cf_charset_idx < 0)
    CFRelease(charset);
  if (charset_string && cf_charset_idx < 0)
...
static Boolean macfont_supports_charset_and_languages_p(CTFontDescriptorRef desc,
                                                        CFCharacterSetRef charset,
                                                        Lisp_Object chars, CFArrayRef languages) {
  Boolean result = true;
      ...
      else /* VECTORP (chars) */
      {
        ...
        for (j = 0; j < ASIZE(chars); j++)
          if (RANGED_FIXNUMP(0, AREF(chars, j), MAX_UNICODE_CHAR) &&
              CFCharacterSetIsLongCharacterMember(desc_charset, XFIXNAT(AREF(chars, j))))
            break;
        if (j == ASIZE(chars))
          result = false;
      }
...
static int macfont_traits_distance(CTFontSymbolicTraits sym_traits1,
...
static Boolean macfont_closest_traits_index_p(CFArrayRef traits_array, CTFontSymbolicTraits target,
                                              CFIndex index) {
  CFIndex i, count = CFArrayGetCount(traits_array);
  CTFontSymbolicTraits traits;
  int my_distance;

  traits = ((CTFontSymbolicTraits)(uintptr_t)CFArrayGetValueAtIndex(traits_array, index));
  my_distance = macfont_traits_distance(target, traits);

  for (i = 0; i < count; i++)
    if (i != index) {
      traits = ((CTFontSymbolicTraits)(uintptr_t)CFArrayGetValueAtIndex(traits_array, i));
...
static Lisp_Object macfont_list(struct frame *f, Lisp_Object spec) {
  ...
  if (languages && (spacing < 0 || spacing == FONT_SPACING_SYNTHETIC_MONO)) {
    CFStringRef language = CFArrayGetValueAtIndex(languages, 0);

    if (CFStringHasPrefix(language, CFSTR("ja")) || CFStringHasPrefix(language, CFSTR("ko")) ||
  ...
  if (family_name)
    families = CFArrayCreate(NULL, (const void **)&family_name, 1, &kCFTypeArrayCallBacks);
  else {
    ...
    if (pref_family) {
      pref_family_index =
          CFArrayGetFirstIndexOfValue(families, CFRangeMake(0, families_count), pref_family);
    ...
    if (pref_family_index > 0) {
      ...
      if (mutable_families) {
        CFArrayAppendValue(mutable_families, CFArrayGetValueAtIndex(families, pref_family_index));
        CFArrayAppendArray(mutable_families, families, CFRangeMake(0, pref_family_index));
        if (pref_family_index + 1 < families_count)
          CFArrayAppendArray(
              mutable_families, families,
              CFRangeMake(pref_family_index + 1, families_count - (pref_family_index + 1)));
        CFRelease(families);
  ...
  for (i = 0; i < families_count; i++) {
    ...
    if (!CFEqual(family_name, CFSTR("LastResort")))
      ...
      CTFontDescriptorRef lr_desc = CTFontDescriptorCreateMatchingFontDescriptor(pat_desc, NULL);
      if (lr_desc) {
        descs = CFArrayCreate(NULL, (const void **)&lr_desc, 1, &kCFTypeArrayCallBacks);
        CFRelease(lr_desc);
      } else
        descs = NULL;
    }
    ...
    if (descs_count == 0 || !macfont_supports_charset_and_languages_p(
                                CFArrayGetValueAtIndex(descs, 0), charset, chars, languages)) {
      CFRelease(descs);
      continue;
    }

    filtered_descs = CFArrayCreateMutable(NULL, descs_count, &kCFTypeArrayCallBacks);
    ...
    for (j = 0; j < descs_count; j++) {
      ...
      num = CFDictionaryGetValue(dict, kCTFontSymbolicTrait);
      CFRelease(dict);
      if (num == NULL || !cfnumber_get_font_symbolic_traits_value(num, &sym_traits))
        continue;

      if (spacing >= 0 && !(synth_sym_traits & kCTFontTraitMonoSpace) &&
          (((sym_traits & kCTFontTraitMonoSpace) != 0) != (spacing >= FONT_SPACING_MONO)))
        continue;
      ...
      if ((sym_traits & kCTFontTraitColorGlyphs) && NILP(family) &&
          !EQ(CDR_SAFE(assq_no_quit(QCscript, AREF(spec, FONT_EXTRA_INDEX))), Qemoji))
        continue;

      if (j > 0 && !macfont_supports_charset_and_languages_p(desc, charset, chars, languages))
        continue;

      CFArrayAppendValue(filtered_descs, desc);
      CFArrayAppendValue(traits_array, (const void *)(uintptr_t)sym_traits);
    }
    ...
    for (j = 0; j < descs_count; j++) {
      CTFontDescriptorRef desc = CFArrayGetValueAtIndex(descs, j);
      CTFontSymbolicTraits sym_traits =
          ((CTFontSymbolicTraits)(uintptr_t)CFArrayGetValueAtIndex(traits_array, j));
      CTFontSymbolicTraits mask_min, mask_max, imask, bmask, mmask;

      mask_min = ((synth_sym_traits ^ sym_traits) & (kCTFontTraitItalic | kCTFontTraitBold));
      ...
      if ((mask_min ^ mask_max) & kCTFontTraitBold) {
        ...
        if (format) {
          ...
          if (CFNumberGetValue(format, kCFNumberSInt32Type, &format_val) &&
      ...
      for (mmask = (mask_min & kCTFontTraitMonoSpace); mmask <= (mask_max & kCTFontTraitMonoSpace);
           mmask += kCTFontTraitMonoSpace)
        for (bmask = (mask_min & kCTFontTraitBold); bmask <= (mask_max & kCTFontTraitBold);
             bmask += kCTFontTraitBold)
          for (imask = (mask_min & kCTFontTraitItalic); imask <= (mask_max & kCTFontTraitItalic);
               imask += kCTFontTraitItalic) {
            CTFontSymbolicTraits synth = (imask | bmask | mmask);

            if (synth == 0 ||
                macfont_closest_traits_index_p(traits_array, (sym_traits | synth), j)) {
...
static Lisp_Object macfont_match(struct frame *frame, Lisp_Object spec) {
  ...
  if (desc) {
    entity = macfont_descriptor_entity(desc, AREF(spec, FONT_EXTRA_INDEX), 0);
    CFRelease(desc);
  }
  unblock_input();
...
static Lisp_Object macfont_list_family(struct frame *frame) {
...
static void macfont_free_entity(Lisp_Object entity) {
  Lisp_Object val = assq_no_quit(QCfont_entity, AREF(entity, FONT_EXTRA_INDEX));
...
static Lisp_Object macfont_open(struct frame *f, Lisp_Object entity, int pixel_size) {
  ...
  font_object = font_build_object(VECSIZE(struct macfont_info), Qmac_ct, entity, size);
  ...
  if (CONSP(val) && EQ(XCDR(val), make_fixnum(1)))
    macfont_info->screen_font = mac_screen_font_create_with_name(font_name, size);
  else
  ...
  if (!(sym_traits & kCTFontTraitBold) && FONT_WEIGHT_NUMERIC(entity) == FONT_WEIGHT_SYNTHETIC_BOLD)
  ...
  if (sym_traits & kCTFontTraitMonoSpace)
    macfont_info->spacing = MACFONT_SPACING_MONO;
           ...
           (XFIXNUM(AREF(entity, FONT_SPACING_INDEX)) == FONT_SPACING_SYNTHETIC_MONO))
  ...
  if (macfont_info->synthetic_italic_p || macfont_info->synthetic_bold_p)
    ...
    if (CONSP(val))
      macfont_info->antialias = NILP(XCDR(val)) ? MACFONT_ANTIALIAS_OFF : MACFONT_ANTIALIAS_ON;
  }
  ...
  if (!(macfont_info->screen_font &&
        mac_screen_font_get_metrics(macfont_info->screen_font, &ascent, &descent, &leading))) {
    ...
    if (family_name) {
      if (CFEqual(family_name, CFSTR("Courier")) || CFEqual(family_name, CFSTR("Helvetica")) ||
...
static void macfont_close(struct font *font) {
  struct macfont_info *macfont_info = (struct macfont_info *)font;
...
static int macfont_has_char(Lisp_Object font, int c) {
  int result;
  CFCharacterSetRef charset;
...
static void macfont_text_extents(struct font *font, const unsigned int *code, int nglyphs,
  ...
  for (i = 1; i < nglyphs; i++) {
    ...
    int w = macfont_glyph_extents(font, code[i], metrics ? &m : NULL, NULL, 0);
...
static int macfont_draw(struct glyph_string *s, int from, int to, int x, int y,
  ...
  if (with_background)
    background_rect = CGRectMake(x, y - FONT_BASE(s->font), s->width, FONT_HEIGHT(s->font));
  else
  ...
  glyphs = xmalloc(sizeof(CGGlyph) * len);
  {
    ...
    for (i = 0; i < len; i++) {
      ...
      width = (s->padding_p ? 1
                            : macfont_glyph_extents(s->font, glyphs[i], NULL, &advance_delta,
                                                    no_antialias_p));
  ...
  if (!CGRectIsNull(background_rect)) {
    ...
    clear_bg_rect([ns_lookup_indexed_color(NS_FACE_BACKGROUND(face), f)
  ...
  if (macfont_info->cgfont) {
    ...
    if (macfont_info->synthetic_bold_p && !no_antialias_p) {
      ...
      if ([[FRAME_NS_VIEW(f) window] respondsToSelector:@selector(backingScaleFactor)])
#endif
    ...
    if (macfont_info->color_bitmap_p
      ...
      if (len > 0) {
        CTFontDrawGlyphs(macfont_info->macfont, glyphs, positions, len, context);
      }
    } else
...
static Lisp_Object macfont_shape(Lisp_Object lgstring, Lisp_Object direction) {
  ...
  string = CFStringCreateWithCharactersNoCopy(NULL, unichars, len + nonbmp_len, kCFAllocatorNull);
  if (string) {
    ...
    if (macfont_info->screen_font)
      used =
          mac_screen_font_shape(macfont_info->screen_font, string, glyph_layouts, glyph_len, dir);
    else
  ...
  for (i = 0; i < used; i++) {
       ...
       the composition is trivial.  */
    {
      ...
      if (unichars[gl->string_index] >= 0xD800 && unichars[gl->string_index] < 0xDC00)
    ...
    if (xoff != 0 || yoff != 0 || wadjust != metrics.width) {
      Lisp_Object vec = CALLN(Fvector, make_fixnum(xoff), make_fixnum(yoff), make_fixnum(wadjust));
...
static CFDataRef mac_font_copy_uvs_table(CTFontRef font) {
  CFDataRef cmap_table, uvs_table = NULL;

  cmap_table = CTFontCopyTable(font, cmapFontTableTag, kCTFontTableOptionNoOptions);
  if (cmap_table) {
    ...
    if (ntables > ((cmap_len - sizeof_sfntCMapHeader) / sizeof_sfntCMapEncoding))
      goto finish;

    for (i = 0; i < ntables; i++)
      if ((BUINT16_VALUE(cmap->encoding[i].platformID) == kFontUnicodePlatform) &&
    ...
    if (i == ntables || uvs_offset > cmap_len || SIZEOF_UVS_TABLE_HEADER > cmap_len - uvs_offset)
      goto finish;

    uvs = (struct uvs_table *)((UInt8 *)cmap + uvs_offset);
    uvs_len = BUINT32_VALUE(uvs->length);
    if (uvs_len > cmap_len - uvs_offset || SIZEOF_UVS_TABLE_HEADER > uvs_len)
      goto finish;
    ...
    if (nrecords > ((uvs_len - SIZEOF_UVS_TABLE_HEADER) / sizeof(struct variation_selector_record)))
      goto finish;
    ...
    for (i = 0; i < nrecords; i++) {
      ...
      if (default_uvs_offset) {
        ...
        if (default_uvs_offset > uvs_len ||
            (SIZEOF_DEFAULT_UVS_TABLE_HEADER > uvs_len - default_uvs_offset))
          goto finish;

        default_uvs = ((struct default_uvs_table *)((UInt8 *)uvs + default_uvs_offset));
        nranges = BUINT32_VALUE(default_uvs->num_unicode_value_ranges);
        if (nranges > ((uvs_len - default_uvs_offset - SIZEOF_DEFAULT_UVS_TABLE_HEADER) /
      ...
      non_default_uvs_offset = BUINT32_VALUE(records[i].non_default_uvs_offset);
      if (non_default_uvs_offset) {
        struct non_default_uvs_table *non_default_uvs;
        UInt32 nmappings;

        if (non_default_uvs_offset > uvs_len ||
            (SIZEOF_NON_DEFAULT_UVS_TABLE_HEADER > uvs_len - non_default_uvs_offset))
          goto finish;

        non_default_uvs = ((struct non_default_uvs_table *)((UInt8 *)uvs + non_default_uvs_offset));
        nmappings = BUINT32_VALUE(non_default_uvs->num_uvs_mappings);
        if (nmappings > ((uvs_len - non_default_uvs_offset - SIZEOF_NON_DEFAULT_UVS_TABLE_HEADER) /
...
static void mac_font_get_glyphs_for_variants(CFDataRef uvs_table, UTF32Char c,
  ...
  while (i < count && ir < nrecords) {
    ...
    non_default_uvs_offset = BUINT32_VALUE(records[ir].non_default_uvs_offset);

    glyphs[i] = kCGFontIndexInvalid;

    if (default_uvs_offset) {
      struct default_uvs_table *default_uvs =
          (struct default_uvs_table *)((UInt8 *)uvs + default_uvs_offset);
      struct unicode_value_range *ranges = default_uvs->unicode_value_ranges;
      UInt32 lo, hi;
      ...
      if (hi > 0 && (c <= (BUINT24_VALUE(ranges[hi - 1].start_unicode_value) +
    ...
    if (glyphs[i] == kCGFontIndexInvalid && non_default_uvs_offset) {
      struct non_default_uvs_table *non_default_uvs =
          (struct non_default_uvs_table *)((UInt8 *)uvs + non_default_uvs_offset);
      ...
      if (hi > 0 && BUINT24_VALUE(mappings[hi - 1].unicode_value) == c)
...
static int macfont_variation_glyphs(struct font *font, int c, unsigned variations[256]) {
  CFDataRef uvs_table;
  ...
  if (uvs_table) {
    ...
    for (i = 0; i < 256; i++) {
      ...
      if (uvs_collection != NSIdentityMappingCharacterCollection && glyph != kCGFontIndexInvalid)
        glyph = macfont_get_glyph_for_cid(font, uvs_collection, glyph);
      if (glyph == kCGFontIndexInvalid)
        variations[i] = 0;
      else {
        variations[i] = (glyph ? glyph : macfont_get_glyph_for_character(font, c));
        n++;
      }
    }
  }
...
static void macfont_filter_properties(Lisp_Object font, Lisp_Object alist) {
  font_filter_properties(font, alist, macfont_booleans, macfont_non_booleans);
}

static Boolean mac_font_descriptor_supports_languages(CTFontDescriptorRef descriptor,
                                                      CFArrayRef languages) {
  Boolean result = true;
  CFArrayRef desc_languages = CTFontDescriptorCopyAttribute(descriptor, kCTFontLanguagesAttribute);
    ...
    for (i = 0; i < languages_count; i++) {
      ...
      if (!CFArrayContainsValue(desc_languages, range, language)
          ...
          && !(CFEqual(language, CFSTR("zh-Hans")) &&
               CFArrayContainsValue(desc_languages, range, CFSTR("zh")))) {
        result = false;
        break;
      }
    }
...
static CFStringRef mac_font_create_preferred_family_for_attributes(CFDictionaryRef attributes) {
  ...
  if (charset_string && CFStringGetLength(charset_string) > 0) {
    ...
    CFArrayRef languages = CFDictionaryGetValue(attributes, kCTFontLanguagesAttribute);

    if (languages && CFArrayGetCount(languages) > 0) {
      if (CTGetCoreTextVersion() >= kCTVersionNumber10_9)
        values[num_values++] = CFArrayGetValueAtIndex(languages, 0);
      else {
        CFCharacterSetRef charset = CFDictionaryGetValue(attributes, kCTFontCharacterSetAttribute);
    ...
    if (result == NULL) {
      ...
      CFDictionaryRef attrs =
          CFDictionaryCreate(NULL, (const void **)keys, (const void **)values, num_values,
      ...
      if (ctline) {
        ...
        for (i = 0; i < nruns; i++) {
          ...
          if (attributes == NULL)
            break;
          font_in_run = CFDictionaryGetValue(attributes, kCTFontAttributeName);
          if (font_in_run == NULL)
            break;
          if (i == 0)
            font = font_in_run;
          else if (!mac_font_equal_in_postscript_name(font, font_in_run))
            break;
        }
        if (nruns > 0 && i == nruns)
...
static inline double mac_font_get_advance_width_for_glyph(CTFontRef font, CGGlyph glyph) {
  return CTFontGetAdvancesForGlyphs(font, kCTFontOrientationDefault, &glyph, NULL, 1);
}

static inline CGRect mac_font_get_bounding_rect_for_glyph(CTFontRef font, CGGlyph glyph) {
  return CTFontGetBoundingRectsForGlyphs(font, kCTFontOrientationDefault, &glyph, NULL, 1);
}
    ...
    if (families)
      for (i = 0; i < count; i++) {
        ...
        if (!CFStringHasPrefix(family, CFSTR(".")) &&
            (CTFontManagerCompareFontFamilyNames(family, CFSTR("LastResort"), NULL) !=
             kCFCompareEqualTo))
...
static Boolean mac_font_equal_in_postscript_name(CTFontRef font1, CTFontRef font2) {
  Boolean result;
...
static CTLineRef mac_font_create_line_with_string_and_font(CFStringRef string, CTFontRef macfont) {
  ...
  if (values[1]) {
    attributes =
        CFDictionaryCreate(NULL, (const void **)keys, (const void **)values, ARRAYELTS(keys),
                           &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
    CFRelease(values[1]);
  }
  if (attributes) {
...
static CFIndex mac_font_shape(CTFontRef font, CFStringRef string,
                              struct mac_glyph_layout *glyph_layouts, CFIndex glyph_len,
                              enum lgstring_direction dir) {
...
#define RIGHT_TO_LEFT_P permutation
      ...
      for (i = 0; i < glyph_count; i++) {
        ...
        if (!RIGHT_TO_LEFT_P)
          ...
          glyph_index = i;
        CTRunGetStringIndices(ctrun, CFRangeMake(glyph_index, 1), &gl->string_index);
        rng = CFStringGetRangeOfComposedCharactersAtIndex(string, gl->string_index);
      ...
      while (1) {
        struct mac_glyph_layout *gl = glbuf + range.location + range.length;

        if (gl->comp_range.length > comp_range.location + comp_range.length)
        ...
        if (min_location >= comp_range.location + comp_range.length) {
          ...
          for (i = 0; i < range.length; i++) {
            ...
            if (RIGHT_TO_LEFT_P)
              permutation[range.location + i] = range.location + range.length - i - 1;
          }
      ...
      /* Then fill the remaining members.  */
      for (range = CFRangeMake(0, 1); range.location < glyph_count; range.location++) {
        struct mac_glyph_layout *gl;
        CGPoint position;
        CGFloat max_x;
        ...
        max_x = position.x + CTRunGetTypographicBounds(ctrun, range, NULL, NULL, NULL);
...
#if USE_CT_GLYPH_INFO
static CGGlyph mac_ctfont_get_glyph_for_cid(CTFontRef font, CTCharacterCollection collection,
                                            CGFontIndex cid) {
  CGGlyph result = kCGFontIndexInvalid;
  UniChar characters[] = {0xfffd};
  CFStringRef string;
  ...
  string = CFStringCreateWithCharacters(NULL, characters, ARRAYELTS(characters));

  if (string) {
    CTGlyphInfoRef glyph_info = CTGlyphInfoCreateWithCharacterIdentifier(cid, collection, string);
    CFDictionaryRef attributes = NULL;

    if (glyph_info) {
      CFStringRef keys[] = {kCTFontAttributeName, kCTGlyphInfoAttributeName};
  ...
  if (ctline) {
    ...
    if (CFArrayGetCount(runs) > 0) {
      ...
      if (attributes) {
        CTFontRef font_in_run = CFDictionaryGetValue(attributes, kCTFontAttributeName);

        if (font_in_run && mac_font_equal_in_postscript_name(font_in_run, font)) {
...
static CFArrayRef mac_font_copy_default_descriptors_for_language(CFStringRef language) {
...
#endif
  {
    CTFontRef user_font = CTFontCreateUIFontForLanguage(kCTFontUIFontUser, 0, language);

    if (user_font) {
      CFArrayRef languages =
          CFArrayCreate(NULL, (const void **)&language, 1, &kCFTypeArrayCallBacks);

      if (languages) {
        result = CTFontCopyDefaultCascadeListForLanguages(user_font, languages);
        CFRelease(languages);
      }
      CFRelease(user_font);
    }
...
#if MAC_OS_X_VERSION_MIN_REQUIRED < 1080
  {
    ...
    for (i = 0; macfont_language_default_font_names[i].language; i++) {
      if (CFEqual(macfont_language_default_font_names[i].language, language)) {
        CFMutableArrayRef descriptors = CFArrayCreateMutable(NULL, 0, &kCFTypeArrayCallBacks);

        if (descriptors) {
          CFIndex j;

          for (j = 0; macfont_language_default_font_names[i].font_names[j]; j++) {
            CFDictionaryRef attributes = CFDictionaryCreate(
                NULL, ((const void **)&kCTFontNameAttribute),
                ((const void **)&macfont_language_default_font_names[i].font_names[j]), 1,
                &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);

            if (attributes) {
              CTFontDescriptorRef pat_desc = CTFontDescriptorCreateWithAttributes(attributes);
...
static CFStringRef mac_font_copy_default_name_for_charset_and_languages(CFCharacterSetRef charset,
                                                                        CFArrayRef languages) {
  CFStringRef result = NULL;
  CFStringRef language = CFArrayGetValueAtIndex(languages, 0);
  CFArrayRef descriptors = mac_font_copy_default_descriptors_for_language(language);
  ...
  if (descriptors) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (macfont_supports_charset_and_languages_p(descriptor, charset, Qnil, languages)) {
        CFStringRef family = CTFontDescriptorCopyAttribute(descriptor, kCTFontFamilyNameAttribute);
        if (family) {
          if (!CFStringHasPrefix(family, CFSTR(".")) && !CFEqual(family, CFSTR("LastResort"))) {
            result = family;
            break;
          } else
            CFRelease(family);
...
void *macfont_get_nsctfont(struct font *font) {
  struct macfont_info *macfont_info = (struct macfont_info *)font;
  CTFontRef macfont = macfont_info->macfont;
...
void mac_register_font_driver(struct frame *f) { register_font_driver(&macfont_driver, f); }
